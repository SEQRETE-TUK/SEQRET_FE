...
export function InventoryQuantityRow({ disabled = false, icon, name, onDecrease, onIncrease, onRemove, onReviewConfirm, quantity, reviewRequired = false }: { disabled?: boolean; icon: ReactNode; name: string; onDecrease: () => void; onIncrease: () => void; onRemove: () => void; onReviewConfirm?: () => void; quantity: number; reviewRequired?: boolean }) {
  return <article className="grid min-h-14 grid-cols-[36px_32px_minmax(0,1fr)] items-center gap-2 ui-card border border-line px-2 py-1.5 shadow-[var(--shadow-card)] min-[360px]:flex">{disabled ? <span className="size-9" /> : <button aria-label={`${name} 삭제`} className="grid size-9 shrink-0 place-items-center rounded-full text-ink-400 hover:bg-surface-muted hover:text-ink-900" onClick={onRemove} type="button"><X aria-hidden="true" size="var(--icon-sm)" /></button>}<span className="grid size-8 shrink-0 place-items-center text-primary-700">{icon}</span><span className="min-w-0 flex-1"><span className="block text-ui-list-title">{name}</span>{reviewRequired ? <span className="mt-0.5 flex items-center gap-2 text-ui-micro !font-bold text-danger-ink"><span>확인 필요</span>{onReviewConfirm ? <button aria-label={`${name} 항목 확인 완료`} className="rounded-full border border-danger-ink/25 px-2 py-0.5 text-[11px] hover:bg-danger-bg disabled:opacity-50" disabled={disabled} onClick={onReviewConfirm} type="button">확인</button> : null}</span> : null}</span><span className="col-span-2 col-start-2 grid shrink-0 grid-cols-[32px_24px_32px] justify-self-end overflow-hidden rounded-md border border-line bg-surface-muted min-[360px]:col-auto min-[360px]:justify-self-auto"><button aria-label={`${name} 수량 줄이기`} className="grid size-8 place-items-center disabled:text-ink-300" disabled={disabled} onClick={onDecrease} type="button"><Minus aria-hidden="true" size="var(--icon-xs)" /></button><output aria-label={`${name} 수량`} className="grid min-h-8 place-items-center bg-surface text-xs font-bold tabular-nums">{quantity}</output><button aria-label={`${name} 수량 늘리기`} className="grid size-8 place-items-center disabled:text-ink-300" disabled={disabled} onClick={onIncrease} type="button"><Plus aria-hidden="true" size="var(--icon-xs)" /></button></span></article>;
}
