...
function Invite({ next, jump }: { next: () => void; jump: (screen: number) => void }) {
  return (
    <div className="flex min-h-[calc(100dvh-48px)] flex-col md:min-h-[796px]">
      ...
      <MobileBottom sub={<button onClick={() => jump(5)} type="button">이 건은 맡지 않을래요</button>}>
        <Button className="w-full" onClick={next} size="cta">수락하고 검토 시작</Button>
      </MobileBottom>
    </div>
  );
}
...
function Quote({ next, back }: { next: () => void; back: () => void }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-48px)] flex-col md:min-h-[796px]">
      ...
      <main className="px-5">
        ...
        <Card className="mt-3 p-4">
          ...
          <div className="mt-3 flex gap-3">
            <button aria-label="현실 영상 재생" className="grid h-20 w-28 place-items-center rounded-xl bg-[#E9EAF2] text-[#4B4B5C]" type="button"><Play className="fill-current" size={28} /></button>
        ...
        <div className="mt-5 flex items-center justify-between"><h2 className={`text-[17px] font-bold ${ink}`}>견적 구성</h2><button className="text-[13px] font-bold text-[#4F46E5]" type="button">품목 21개 보기</button></div>
        <Card className="mt-2 p-4">
          ...
          <button className="mt-3 text-[13px] font-bold text-[#4F46E5]" type="button"><Plus className="inline" size={15} /> 항목 추가</button>
      ...
      <MobileBottom>
        <div className="grid grid-cols-[1fr_104px] gap-2"><Button onClick={next} size="cta">견적 제안 보내기</Button><Button size="cta" variant="outline">임시 저장</Button></div>
      </MobileBottom>
    </div>
  );
}
...
function Revision({ next, back }: { next: () => void; back: () => void }) {
  return (
    <div className="flex min-h-[calc(100dvh-48px)] flex-col bg-[#191927]/35 pt-20 md:min-h-[796px]">
...
function Assignment({ next, back }: { next: () => void; back: () => void }) {
  ...
  return (
    <div className="flex min-h-[calc(100dvh-48px)] flex-col md:min-h-[796px]">
      ...
      <MobileBottom sub={<button onClick={() => setSent(false)} type="button">인원 바꾸기</button>}><Button className="w-full" onClick={() => { setSent(true); next(); }} size="cta">배정 확정 · 링크 발송</Button></MobileBottom>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-[calc(100dvh-48px)] flex-col md:min-h-[796px]">
      ...
      <main className="px-5">
        ...
        <Card className="mt-4 border-[#F5A623] bg-[#FFF6E5] p-4"><Badge variant="warning">지금 처리할 1건</Badge><h2 className={`mt-2 text-[16px] font-bold ${ink}`}>사다리차 +150,000원 · CR-01</h2><p className={`mt-1 text-xs ${muted}`}>김도윤 기사 10:55 · 고객 응답 대기 12분째</p><div className="mt-3 grid grid-cols-2 gap-2"><Button size="chip" variant="outline">요청 상세 · 증빙 2</Button><Button onClick={() => setReminded(true)} size="chip" variant="outline">고객에게 리마인드</Button></div></Card>
        ...
        </div></Card>{reminded && <p className="mt-3 text-center text-xs font-bold text-[#17A46B]">고객에게 리마인드 알림을 보냈어요.</p>}
      </main>
      <MobileBottom><div className="grid grid-cols-2 gap-2"><Button variant="secondary">작업자 현황</Button><Button onClick={next} variant="outline">완료 기록 준비</Button></div></MobileBottom>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-[calc(100dvh-48px)] flex-col md:min-h-[796px]">
      ...
      <main className="px-5">
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">{[['거실 5장',true],['침실 6장',true],['주방·베란다',false]].map(([label,done]) => <button className={`grid h-24 place-items-center rounded-xl border ${done ? 'border-transparent bg-[#E9EAF2]' : 'border-dashed border-[#F5A623] bg-[#FFF6E5]'}`} key={String(label)} type="button">{done ? <Check className="rounded-full bg-[#17A46B] p-1 text-white" /> : <Video className="text-[#F5A623]" />}<span className={`-mt-3 text-[11px] font-bold ${done ? ink : 'text-[#9A6200]'}`}>{label}</span></button>)}</div>
        ...
        <div className="mt-5 flex items-center gap-3 px-5"><FileText className={muted} /><div className="flex-1"><b className={`text-[13px] ${ink}`}>문서 패키지 · 30일 보관</b><p className={`text-xs ${muted}`}>견적서 v3 · 변경 승인서 · 완료 확인서</p></div><Button size="chip" variant="outline"><Download size={15} /> PDF</Button></div>
      </main>
      <MobileBottom sub="고객이 확인하면 기록이 잠기고 정산으로 넘어가요"><Button className="w-full" onClick={() => setRequested(true)} size="cta">{requested ? '완료 확인 요청 보냄' : '완료 확인 요청 보내기'}</Button></MobileBottom>
    </div>
  );
}
...
function WebShell({ view, setView, children }: { view: WebView; setView: (view: WebView) => void; children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F4F5F9] text-[#191927]">
      <header className="flex h-16 items-center border-b border-[#E9EAF2] bg-white px-7"><strong className="text-2xl font-black text-[#3730A3]">짐싸</strong><Badge className="ml-4" variant="primary">파트너 PWA</Badge><button className={`ml-auto text-[13px] font-bold ${muted}`} type="button">한빛이사 · 이상담 관리자⌄</button></header>
