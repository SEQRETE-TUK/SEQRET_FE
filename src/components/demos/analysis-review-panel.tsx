    ...
    <form id={ANALYSIS_REVIEW_FORM_ID} onSubmit={submit}>
      <Card className="mt-5 overflow-hidden border-primary-100">
        ...
        <div className="space-y-3 p-4">
              ...
              <article className="rounded-2xl border border-line bg-white p-4" key={draft.itemKey}>
                ...
                <label className="mt-4 block text-[11px] font-bold text-ink-400">
                  ...
                  <select
                    ...
                    className="mt-1.5 h-11 w-full rounded-xl border border-line bg-white px-3 text-[13px] font-bold outline-none focus:border-primary-600 disabled:bg-[#F5F6F8]"
                    disabled={completed}
          ...
          {!completed && (
            <Button className="w-full" onClick={onAdd} size="chip" type="button" variant="secondary">
              <Plus size={17} /> 빠진 항목 직접 추가
            </Button>
          )}
        </div>
      </Card>
