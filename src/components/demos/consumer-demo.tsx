  ...
  const content = (
    <>
      <div className="grid flex-1 grid-cols-[1fr_40px_1fr] items-center gap-2 text-center">
        <div><div className="flex items-center justify-center gap-2 text-[12px] font-bold text-ink-400"><MapPin size={16} />출발지</div><p className="mt-2 whitespace-nowrap text-[16px] font-extrabold">마포구 성산동</p><p className="mt-1 whitespace-nowrap text-[12px] text-ink-600">12층 · 엘리베이터</p></div>
        <span className="grid size-9 place-items-center rounded-full bg-canvas text-ink-600"><ArrowRight size={18} /></span>
        <div><div className="flex items-center justify-center gap-2 text-[12px] font-bold text-ink-400"><MapPin size={16} />목적지</div><p className="mt-2 whitespace-nowrap text-[16px] font-extrabold">{destination}</p><p className="mt-1 whitespace-nowrap text-[12px] text-ink-600">{destinationDetail}</p></div>
      </div>
  ...
  return (
    <Page>
      ...
      <main className="flex-1 px-6 pb-28 pt-3">
        ...
        <h1 className="mb-4 whitespace-nowrap text-[22px] font-extrabold leading-9 tracking-[-0.5px]">{task.headline}</h1>
        <Card className="mb-5 overflow-hidden border-primary-100">
          <div className="p-5">
            <p className="text-[12px] font-bold text-ink-400">지금 확인할 일</p>
            <h2 className="mt-2 whitespace-nowrap text-[20px] font-extrabold leading-7">{task.title}</h2>
            <div className="mt-4 flex items-center justify-between border-t border-line pt-4 text-[13px]"><span className="text-ink-600">{task.detail}</span><strong className={task.danger ? "text-danger-ink" : "text-ink-900"}>{task.amount}</strong></div>
          </div>
          <button onClick={() => go(task.target)} className="flex h-14 w-full items-center justify-between bg-primary-600 px-5 text-[15px] font-bold text-white">{task.cta} <ArrowRight size={18} /></button>
        </Card>
        <div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="whitespace-nowrap text-[18px] font-bold">{task.date}</h2><p className="mt-1 text-[12px] text-ink-400">{task.dateDetail}</p></div><p className={`flex whitespace-nowrap text-[13px] font-bold ${phase === "closed" ? "text-success-ink" : "text-warning-ink"}`}>{task.status}</p></div>
        <Card className="p-5">
          <MoveRoute onClick={() => go(14)} destination={destination} destinationDetail={destinationDetail} />
          <div className="mt-5 flex items-end justify-between border-t border-line pt-4"><span className="text-[12px] font-semibold text-ink-400">{phase === "quote" ? "현재 제안 금액" : "현재 기록 금액"}</span><strong className="text-[20px]">{task.price}</strong></div>
        </Card>
        {phase === "quote" && <section className="mt-5">
          ...
          <div className="space-y-3">
            {quotes.filter((item) => item.name !== quote.name).map((item) => <button key={item.name} onClick={() => go(10)} className="flex min-h-[76px] w-full items-center rounded-2xl bg-white px-5 text-left"><span className="grid size-10 place-items-center rounded-xl bg-canvas text-[12px] font-extrabold text-ink-600">{item.name.slice(0, 2)}</span><span className="ml-3 flex-1"><b className="block text-[15px]">{item.name}</b><small className="mt-1 block text-[12px] text-ink-400">{item.detail}</small></span><strong className="text-[16px]">{item.price}</strong></button>)}
          </div>
        </section>}
      </main>
    ...
    <Page>
      ...
      <main className="flex-1 px-6 pb-28 pt-3">
        ...
        <section className="mt-7">
          <p className="text-[12px] font-bold text-ink-400">{phase === "closed" ? "진행 상태" : "다음 할 일"}</p>
          {phase === "closed" ? <Card className="mt-2 border-success p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-full bg-success-bg text-success-ink"><Check size={22} /></span><div><h3 className="text-[18px] font-bold">이사가 완료됐어요</h3><p className="mt-1 text-[13px] text-ink-600">최종 금액 {quote.finalPrice} · 9월 12일 14:28</p></div></div></Card> : <Card className="mt-2 overflow-hidden border-primary-100"><div className="p-5"><h3 className="text-[18px] font-bold">{nextAction.title}</h3><p className="mt-1 text-[13px] text-ink-600">{nextAction.detail}</p></div><button onClick={() => go(nextAction.target)} className="flex h-14 w-full items-center justify-between bg-primary-600 px-5 text-[15px] font-bold text-white">{nextAction.cta} <ArrowRight size={18} /></button></Card>}
        </section>
  ...
  return (
    <Page>
      ...
      <main className="flex-1 px-6 pb-5 pt-5">
        ...
        {editingAddress ? <Card className="mt-5 space-y-3 p-5"><label className="block text-[12px] font-bold text-ink-600">도착지 주소<input value={draftAddress} onChange={(event) => setDraftAddress(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-line bg-white px-4 text-[14px] outline-none focus:border-primary-600" /></label><label className="block text-[12px] font-bold text-ink-600">주거 형태·층수<input value={draftFloor} onChange={(event) => setDraftFloor(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-line bg-white px-4 text-[14px] outline-none focus:border-primary-600" /></label><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => setEditingAddress(false)}>취소</Button><Button disabled={!draftAddress.trim() || !draftFloor.trim()} onClick={() => { const nextAddress = draftAddress.trim(); const nextFloor = draftFloor.trim(); setAddress(nextAddress); setFloor(nextFloor); onDestinationChange(nextAddress, nextFloor); setEditingAddress(false); }}>주소 저장</Button></div></Card> : <Card className="mt-5 flex items-center gap-3 p-5"><span className="grid size-8 place-items-center rounded-full bg-canvas"><MapPin size={20} /></span><div><p className="text-[14px] font-bold">{address} · {floor}</p><p className={`text-[12px] ${muted}`}>9월 12일 (토) 오전 8시 도착 예정</p></div><button onClick={() => { setDraftAddress(address); setDraftFloor(floor); setEditingAddress(true); }} className="ml-auto inline-flex min-h-11 min-w-11 items-center justify-center px-2 text-[12px] font-bold">변경</button></Card>}
        <h2 className="mb-2 mt-7 text-[15px] font-bold">엘리베이터가 있나요?</h2>{choices(["있어요", "없어요", "모름"], elevator, setElevator)}
        {elevator === "없어요" && <p className="mt-2 rounded-xl bg-warning-bg px-4 py-3 text-[12px] font-bold text-warning-ink">{floor.includes("계단") ? floor : `${floor} · 계단`} 작업 예상 — 사다리차 여부를 이어서 확인할게요</p>}
...
function ConfirmItems({ next, back, openItems }: { next: () => void; back: () => void; openItems: () => void }) {
  ...
  return (
    <Page><StatusBar /><Top onBack={back} title="짐 목록 확인" aside={done ? "완료" : `${step}/2`} /><main className="flex-1 px-6 pb-5"><div className="flex items-center justify-between gap-3"><div><h1 className="whitespace-nowrap text-[24px] font-extrabold leading-[30px]">확인 2건이면 짐 목록이 끝나요</h1><p className={`mt-1 text-[13px] ${muted}`}>영상에서 판단이 어려운 항목만 확인해요</p></div></div>
        {done ? <section className={`${card} demo-pop mt-5 p-6 text-center`}><span className="mx-auto grid size-14 place-items-center rounded-full bg-success-bg text-success-ink"><Check size={28} /></span><h2 className="mt-4 text-[24px] font-extrabold">확인이 모두 끝났어요</h2><p className="mt-2 text-[16px] font-semibold text-ink-600">짐 목록 21개를 업체 견적에 사용할 수 있어요</p></section> : <section className={`${card} mt-5 p-5`}><span className="text-[12px] font-bold text-warning-ink">확인 {step}/2</span><h2 className="mt-3 text-[26px] font-extrabold leading-8">{step === 1 ? "붙박이장인가요?" : "화분도 가져가나요?"}</h2><p className="mt-2 text-[17px] font-bold leading-6 text-ink-900">{step === 1 ? "벽에 고정된 장은 운반에서 제외돼요" : "이삿짐에 포함할지 알려주세요"}</p><button aria-label={playingStep === step ? "근거 영상 일시정지" : "근거 영상 재생"} onClick={() => setPlayingStep(playingStep === step ? null : step)} className="relative mt-4 h-36 w-full overflow-hidden rounded-2xl bg-canvas"><Image src={step === 1 ? "/built-in-wardrobe-evidence.png" : "/large-plant-evidence.png"} alt={step === 1 ? "거실 벽에 고정된 붙박이장 영상 장면" : "거실 창가의 대형 화분 영상 장면"} fill sizes="320px" className="object-cover" /><span className="absolute inset-0 grid place-items-center bg-ink-900/15"><span className="grid size-11 place-items-center rounded-full bg-white text-ink-900">{playingStep === step ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" />}</span></span><span className="absolute bottom-3 left-3 rounded-lg bg-ink-900/80 px-2 py-1 text-[11px] font-bold text-white">{playingStep === step ? "재생 중 · 0:08" : `AI 판단 확률 ${step === 1 ? 61 : 72}%`}</span></button><div className="mt-5 grid grid-cols-2 gap-2"><button onClick={answer} className="h-14 rounded-2xl bg-primary-600 text-[16px] font-bold text-white">맞아요</button><button onClick={answer} className="h-14 rounded-2xl border border-line text-[16px] font-bold">아니요</button></div>{step === 1 && <p className={`mt-3 text-center text-[12px] ${muted}`}>다음 확인 · 대형 화분</p>}</section>}
...
function ScopeSummary({ next, back, go, company, accepted, revisionRequested, demoState = "" }: { next: () => void; back: () => void; go: (screen: number) => void; company: string; accepted: boolean; revisionRequested: boolean; demoState?: string }) {
  ...
  return (
    <Page><StatusBar /><Top onBack={back} title={`${quote.name} 견적 ${quote.version}`} aside={<button className="font-bold text-primary-700" onClick={() => go(12)}>이전 견적 비교</button>} /><main className="flex-1 px-6 pb-5"><h1 className="mb-5 mt-3 whitespace-nowrap text-[26px] font-extrabold leading-[34px] tracking-[-0.5px]">{revisionRequested ? "수정 견적을 기다리고 있어요" : "이 견적으로 진행할까요?"}</h1><Card className="p-5"><div className="flex items-center justify-between"><span className={`text-[14px] font-semibold ${muted}`}>제안 총액</span><span className={`text-[13px] font-bold ${confirmed ? "text-success-ink" : "text-warning-ink"}`}>{confirmed ? "수락 완료" : revisionRequested ? "업체 답변 대기" : "내 수락 대기"}</span></div><p className="mt-2 text-[32px] font-extrabold tracking-[-0.5px]">{quote.price}</p><div className="mt-4 flex items-center justify-between border-t border-line pt-4"><span className="text-[13px] text-ink-600">AI 예상 견적 대비</span><strong className="text-[16px] text-danger-ink">{quote.delta}</strong></div><p className={`mt-4 text-[14px] leading-6 ${muted}`}>{quote.detail} · 예상 6시간</p></Card>
        <h2 className="mb-3 mt-7 text-[17px] font-bold">이번에 달라진 것</h2><Card className="p-5"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-canvas text-ink-600"><Plus size={21} /></span><div className="min-w-0 flex-1"><p className="text-[16px] font-bold">{quote.reason}</p><p className="mt-1 text-[13px] text-ink-600">업체가 제출한 변경 사유예요</p></div><strong className="text-[16px] text-danger-ink">{quote.delta}</strong></div>{quote.name === "한빛이사" && <><Button onClick={() => setShowEvidence(!showEvidence)} variant="outline" className="mt-4 w-full"><Play size={17} />{showEvidence ? "영상 근거 닫기" : "영상 근거 보기"}</Button>{showEvidence && <button onClick={() => notify("피아노 운반 근거 영상을 재생했어요.")} className="relative mt-3 h-32 w-full overflow-hidden rounded-2xl"><Image src="/upright-piano-evidence.png" alt="피아노 전문 운반이 필요한 근거 영상" fill sizes="320px" className="object-cover" /><span className="absolute inset-0 grid place-items-center bg-ink-900/20"><span className="grid size-11 place-items-center rounded-full bg-white"><Play size={20} fill="currentColor" /></span></span></button>}</>}</Card>
        ...
        {revisionRequested && <Card className="mt-5 border-warning bg-warning-bg p-4"><p className="text-[14px] font-bold text-warning-ink">수정 요청을 보냈어요</p><p className="mt-1 text-[13px] text-ink-600">{quote.name}가 수정 내용을 반영한 새 견적을 보내면 알려드릴게요.</p></Card>}
        <h2 className="mb-3 mt-7 text-[17px] font-bold">수락 상태</h2><section className={`${card} space-y-2 p-5`}><label className="flex min-h-11 items-center gap-3"><input aria-label={`${quote.name} 수락 완료`} type="checkbox" checked readOnly className="size-5 accent-[#17A46B]" /><span><b className="block text-[15px]">업체 · {quote.name}</b><small className="text-[12px] text-ink-400">이 견적으로 진행 요청함</small></span></label><label className="flex min-h-11 items-center gap-3"><input aria-label="박민서 고객 수락 상태" type="checkbox" checked={confirmed} readOnly className="size-5 accent-[#17A46B]" /><span><b className="block text-[15px]">나 · 박민서</b><small className="text-[12px] text-ink-400">{confirmed ? "수락 완료" : revisionRequested ? "수정 요청 보냄" : "수락 대기"}</small></span></label><p className="border-t border-line pt-4 text-[13px] leading-5 text-ink-600">{confirmed ? "양쪽이 수락해 이 견적으로 진행해요." : revisionRequested ? "수정된 새 견적을 기다리고 있어요." : "내가 수락하면 이 견적으로 이사를 진행해요."}</p></section>
        {confirmed && <div className="demo-pop mt-4 rounded-2xl bg-success-bg p-4"><p className="text-[14px] font-bold text-success-ink">양측 수락 완료</p><p className={`mt-1 text-[13px] ${muted}`}>업체가 이 견적 기준으로 배차와 인력을 확정할 수 있어요.</p></div>}
...
function OnsiteApproval({ next, back, company, demoState = "" }: { next: () => void; back: () => void; company: string; demoState?: string }) {
  ...
  if (requestCancelled || requestExpired || explanationPending) {
    ...
    return (
      <Page><StatusBar /><Top onBack={back} title="현장 변경" aside={<Badge variant="warning">{requestCancelled ? "CANCELLED" : requestExpired ? "EXPIRED" : "설명 요청됨"}</Badge>} />
        <main className="flex-1 px-6 pb-5"><div className="demo-pop rounded-2xl border border-warning bg-warning-bg p-5"><AlertTriangle className="text-warning" size={24} /><h1 className="mt-4 text-[21px] font-extrabold">{title}</h1><p className={`mt-2 text-[13px] leading-5 ${muted}`}>{description}</p></div><Card className="mt-4 p-5"><div className="flex justify-between text-[13px]"><span className={muted}>기준 양측 수락 견적</span><b>{quote.version} · 유지</b></div><div className="mt-3 flex justify-between text-[13px]"><span className={muted}>현재 확정 금액</span><b>{quote.price}</b></div><div className="mt-3 flex justify-between text-[13px]"><span className={muted}>요청 증감액</span><b className="text-ink-400">+150,000원 · 미반영</b></div></Card>{explanationPending && <Card className="mt-4 border-primary-400 bg-primary-50 p-4"><p className="text-[13px] font-bold text-primary-600">내 설명 요청 · 11:04</p><p className={`mt-1 text-xs ${muted}`}>“사다리차가 꼭 필요한 근거와 작업 가능 위치를 더 알려주세요.”</p><p className="mt-3 text-xs font-bold">업체 답변 또는 수정 견적 대기</p></Card>}</main>
  ...
  return (
    <Page><StatusBar /><div className="px-6 pt-4"><h2 className="text-[17px] font-bold">오늘 · 이사 진행 중</h2><section className="mt-3 rounded-2xl bg-white/60 p-4"><p className="text-[14px] font-bold">09:40 상차 완료 · 도착지 이동 중</p><p className={`text-[12px] ${muted}`}>{quote.name} 김도윤 팀 · 확정 {quote.price}</p></section></div><div className="mt-4 flex flex-1 flex-col rounded-t-[28px] bg-white px-6 pb-7 pt-3"><div className="mx-auto mb-5 h-1 w-12 rounded-full bg-[#DFE1EA]" /><div className="flex items-center justify-between"><span className={`rounded-full px-3 py-2 text-[11px] font-bold ${approved ? "bg-success-bg text-success-ink" : "bg-danger-bg text-danger-ink"}`}>{approved ? `승인 완료 · ${quote.finalVersion}` : "현장 추가 요청"}</span><button aria-label="닫기" className="grid size-11 place-items-center rounded-full" onClick={back}><X size={22} className="text-ink-400" /></button></div><h1 className="mt-4 text-[22px] font-extrabold leading-[30px]">{approved ? "변경 금액이 반영됐어요" : <>사다리차 150,000원,<br />승인하시겠어요?</>}</h1><p className={`mt-1 text-[13px] ${muted}`}>김도윤 기사 · 10:55 · 도착지 엘리베이터 고장</p>{response && <p className="mt-3 rounded-xl bg-warning-bg px-4 py-3 text-[12px] font-bold text-warning-ink">{response} · 기존 양측 수락 견적 {quote.version}과 {quote.price}은 그대로 유지돼요</p>}<div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-canvas p-5"><p className={`text-[12px] ${muted}`}>변경 전</p><strong className="mt-3 block text-[20px]">{quote.price}</strong><p className={`mt-3 text-[11px] ${muted}`}>양측 수락 견적 {quote.version}</p></div><div className="rounded-2xl border-2 border-primary-600 bg-primary-50 p-5"><p className="text-[12px] font-bold text-primary-600">변경 후</p><strong className="mt-3 block text-[20px] text-primary-600">{quote.finalPrice}</strong><p className={`mt-3 text-[11px] ${muted}`}>+150,000원 · {quote.finalVersion}</p></div></div><h2 className="mb-2 mt-5 text-[15px] font-bold">현장 증빙 2건</h2><div className="grid grid-cols-2 gap-3">{['고장 엘리베이터','고장 안내문'].map((label, i) => <button onClick={() => notify(`${label} 증빙을 크게 열었어요.`)} key={label} className="demo-interactive-card relative h-28 overflow-hidden rounded-2xl"><Image alt={label} className="object-cover" fill sizes="160px" src="/elevator-outage-evidence.png" style={{ objectPosition: i ? "50% 46%" : "34% 50%" }} /><span className="absolute inset-x-0 bottom-0 bg-ink-900/75 px-3 py-2 text-left text-[11px] font-bold text-white">{label}</span></button>)}</div><p className="mt-4 text-[12px] font-semibold text-ink-600">구두 동의는 승인으로 기록되지 않아요 · 여기서 응답해야 반영돼요</p>{approved && <Card className="demo-pop mt-4 border-success p-4"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-success-bg text-success-ink"><Check size={22} /></span><div><p className="text-[15px] font-bold">변경 승인이 기록됐어요</p><p className={`mt-1 text-[12px] ${muted}`}>작업자는 {quote.finalVersion} 범위로 작업을 계속해요.</p></div></div></Card>}<div className="mt-auto">{approved ? <Primary onClick={next}>완료 기록 확인</Primary> : <><Primary disabled={submitting} onClick={() => { if (submitting) return; setSubmitting(true); window.setTimeout(() => { setApproved(true); setSubmitting(false); notify(`변경을 승인했고 ${quote.finalVersion}가 생성됐어요.`); }, 550); }}>{submitting ? <><LoaderCircle className="demo-spin mr-2 inline" size={18} />처리 중...</> : `승인하기 · ${quote.finalPrice}`}</Primary><div className="mt-3 grid grid-cols-2 gap-2"><Outline onClick={() => setDecisionMode("explain")}>설명 요청</Outline><button onClick={() => setDecisionMode("reject")} className="h-14 rounded-2xl border border-danger font-bold text-danger-ink">거절하기</button></div></>}</div></div>
      ...
      {sheet && (
        <Sheet open onOpenChange={(open) => !open && back()}>
          <SheetContent>
            ...
            <div className="px-5">
              {sofa > 0 ? <Card className="border-primary-100 bg-primary-50 p-4"><div className="flex gap-3"><span className="grid size-11 place-items-center rounded-xl bg-white"><Sofa size={25} /></span><div><h2 className="text-[17px] font-bold">3인 소파</h2><p className="text-[12px]">{sofaOption} · AI 93% · 근거 0:08</p></div><div className="ml-auto flex h-11 items-center rounded-xl bg-white"><button className="grid size-11 place-items-center" aria-label="소파 수량 줄이기" onClick={() => setSofa(Math.max(0, sofa - 1))}><Minus size={16} /></button><span>{sofa}</span><button className="grid size-11 place-items-center" aria-label="소파 수량 늘리기" onClick={() => setSofa(sofa + 1)}><Plus size={16} /></button></div></div><div className="mt-4 grid grid-cols-2 gap-2"><Outline onClick={() => setSofaOption(sofaOption === "일반 운반" ? "업체 포장" : "일반 운반")}>옵션 · {sofaOption}</Outline><Outline danger onClick={() => setSofa(0)}>이 짐 빼기</Outline></div></Card> : <button onClick={() => setSofa(1)} className="w-full rounded-2xl bg-danger-bg p-4 text-[13px] font-bold text-danger-ink">소파 제외됨 · 되돌리기</button>}
              ...
              <Button onClick={() => setExpandedItems(!expandedItems)} variant="outline" className="mt-3 w-full justify-between"><span>책장 · 러그 · 스탠드 · 3개</span><ChevronDown className={expandedItems ? "rotate-180" : ""} /></Button>
              ...
              {customItems.map((item) => <div key={item} className="mt-2 flex min-h-12 items-center rounded-2xl bg-white px-4"><b className="flex-1">{item}</b><button onClick={() => setCustomItems(customItems.filter((name) => name !== item))} aria-label={`${item} 삭제`} className="grid size-11 place-items-center text-danger-ink"><X size={18} /></button></div>)}
              {addingItem ? <Card className="mt-3 space-y-3 p-4"><label className="block text-[12px] font-bold text-ink-600">카테고리<select value={itemCategory} onChange={(event) => { const category = event.target.value as keyof typeof itemCatalog; setItemCategory(category); setItemDetail(itemCatalog[category][0]); }} className="mt-2 h-12 w-full rounded-xl border border-line bg-white px-3 text-[14px] font-bold outline-none focus:border-primary-600">{Object.keys(itemCatalog).map((category) => <option key={category}>{category}</option>)}</select></label><label className="block text-[12px] font-bold text-ink-600">세부 품목<select value={itemDetail} onChange={(event) => setItemDetail(event.target.value)} className="mt-2 h-12 w-full rounded-xl border border-line bg-white px-3 text-[14px] font-bold outline-none focus:border-primary-600">{itemCatalog[itemCategory].map((item) => <option key={item}>{item}</option>)}</select></label><div className="grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => setAddingItem(false)}>취소</Button><Button onClick={() => { if (!customItems.includes(itemDetail)) setCustomItems([...customItems, itemDetail]); setAddingItem(false); }}>목록에 추가</Button></div></Card> : <Button onClick={() => setAddingItem(true)} variant="outline" className="mt-3 w-full border-dashed"><Plus />이 공간에 짐 추가</Button>}
            </div>
...
function Invite({ next, back, selected, onSelect }: { next: () => void; back: () => void; selected: string; onSelect: (company: string) => void }) {
  return (
    <Page>
      ...
      <main className="flex-1 px-6 pb-5">
        <h1 className="mt-4 whitespace-nowrap text-[26px] font-extrabold leading-[34px]">3개 업체가 견적을 보냈어요</h1>
        ...
        <div className="mt-5 space-y-3">
          {quotes.map(({ name, price, detail, delta, label }) => {
            ...
            return <button key={name} onClick={() => onSelect(name)} className={`w-full rounded-2xl border-2 bg-white p-5 text-left ${active ? "border-primary-600" : "border-transparent"}`}><div className="flex items-start justify-between"><div><span className={`text-[12px] font-bold ${label === "추천" ? "text-primary-700" : "text-ink-400"}`}>{label}</span><h2 className="mt-1 text-[18px] font-extrabold">{name}</h2></div><span className={`grid size-6 place-items-center rounded-full border-2 ${active ? "border-primary-600 bg-primary-600 text-white" : "border-line"}`}>{active && <Check size={15} />}</span></div><p className="mt-4 text-[24px] font-extrabold">{price}</p><div className="mt-3 flex items-center justify-between text-[13px]"><span className="text-ink-600">{detail}</span><strong className="text-danger-ink">{delta}</strong></div></button>;
          })}
        </div>
...
function History({ back, go, company, phase }: { back: () => void; go: (screen: number) => void; company: string; phase: MovePhase }) {
  ...
  const versionCard = (item: (typeof activeHistory)[number]) => {
    ...
    return <Card key={item.id} className={open ? "border-primary-400" : ""}><button onClick={() => setSelected(open ? "" : item.id)} className="flex min-h-[76px] w-full items-center p-4 text-left"><span className={`grid size-10 shrink-0 place-items-center rounded-xl text-[12px] font-extrabold ${accepted && item.id === activeHistory[0].id ? "bg-success-bg text-success-ink" : "bg-canvas text-ink-600"}`}>{item.version}</span><span className="ml-3 min-w-0 flex-1"><b className="block text-[15px]">{item.round}</b><small className="mt-1 block text-[12px] text-ink-400">{item.time} · {item.price}</small></span><ChevronDown className={`ml-2 shrink-0 text-ink-400 ${open ? "rotate-180" : ""}`} size={18} /></button>{open && <div className="border-t border-line px-4 pb-4 pt-3"><p className="text-[13px] leading-5 text-ink-600">{item.summary}</p><button onClick={() => go(item.target)} className="mt-3 min-h-11 text-[13px] font-bold text-primary-700">견적 내용 보기</button></div>}</Card>;
  };
  ...
  return (
    <Page><StatusBar /><Top onBack={back} title="견적과 변경 기록" /><main className="flex-1 px-6 pb-32 pt-2"><p className={`text-[14px] leading-6 ${muted}`}>업체를 선택하면 해당 업체가 보낸 견적만 보여요.</p><div className="mt-4 flex gap-2 overflow-x-auto pb-1">{quotes.map((quote) => <button key={quote.name} onClick={() => { const history = providerHistories[quote.name]; const revealFinal = quote.name === company && (phase === "completed" || phase === "closed"); setActiveCompany(quote.name); setSelected(history[revealFinal ? 0 : 1].id); }} className={`h-11 shrink-0 rounded-full px-4 text-[12px] font-bold ${activeCompany === quote.name ? "bg-ink-900 text-white" : "bg-white text-ink-600"}`}>{quote.name}</button>)}</div>
      <Card className={`mt-4 bg-white p-5 ${accepted ? "border-success" : reviewing ? "border-warning" : "border-line"}`}><div className={`flex items-center gap-2 text-[13px] font-bold ${accepted ? "text-success-ink" : reviewing ? "text-warning-ink" : "text-ink-600"}`}>{accepted ? <CheckCircle2 size={18} /> : <Info size={18} />}{accepted ? "현재 양측이 수락한 견적" : reviewing ? "현재 검토 중인 견적" : "선택하지 않은 업체의 최종 견적"} · {activeHistory[0].version}</div><strong className="mt-3 block text-[30px] font-extrabold">{activeHistory[0].price}</strong><p className="mt-2 text-[13px] leading-5 text-ink-600">{accepted ? `${activeCompany}와 나(박민서)가 모두 수락했어요.` : reviewing ? `${activeCompany}가 진행을 요청했고 내 수락을 기다려요.` : `${activeCompany}가 마지막으로 보낸 견적이에요.`}</p></Card>
      <div className="mt-5 flex items-center justify-between"><h2 className="text-[17px] font-bold">{activeCompany} 견적 {activeHistory.length}회</h2><button onClick={downloadHistory} className="inline-flex min-h-11 items-center gap-2 px-2 text-[12px] font-bold text-primary-700"><FileDown size={17} />기록 저장</button></div><div className="mt-3 space-y-3">{activeHistory.map(versionCard)}</div>
      <h2 className="mb-3 mt-7 text-[17px] font-bold">짐·조건 변경 기록</h2><div className="space-y-3">{changes.map(([id, title, summary, time, target]) => { const open = selected === id; return <Card key={id} className={open ? "border-primary-400" : ""}><button onClick={() => setSelected(open ? "" : id)} className="flex min-h-[76px] w-full items-center p-4 text-left"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-canvas text-ink-600"><ClipboardList size={19} /></span><span className="ml-3 min-w-0 flex-1"><b className="block text-[15px]">{title}</b><small className="mt-1 block text-[12px] text-ink-400">{time}</small></span><ChevronDown className={`ml-2 shrink-0 text-ink-400 ${open ? "rotate-180" : ""}`} size={18} /></button>{open && <div className="border-t border-line px-4 pb-4 pt-3"><p className="text-[13px] leading-5 text-ink-600">{summary}</p><button onClick={() => go(target)} className="mt-3 min-h-11 text-[13px] font-bold text-primary-700">변경 내용 보기</button></div>}</Card>; })}</div>
...
function Profile({ go, phase, company, destination }: { go: (screen: number) => void; phase: MovePhase; company: string; destination: string }) {
  ...
  return (
    <Page><StatusBar /><header className="flex h-14 items-center px-6"><h1 className="text-[22px] font-extrabold">내 정보</h1></header><main className="flex-1 px-6 pb-28 pt-3">
      <Card className="flex items-center p-5"><span className="grid size-14 place-items-center rounded-full bg-primary-50 text-primary-700"><UserRound size={28} /></span><div className="ml-4"><h2 className="text-[20px] font-extrabold">{name}</h2><p className="mt-1 text-[13px] text-ink-400">{phone.replace(/(010)-\d{4}-(\d{4})/, "$1-****-$2")}</p></div><button onClick={() => { setDraftName(name); setDraftPhone(phone); setPanel("edit"); }} className="ml-auto min-h-11 min-w-12 px-2 text-[13px] font-bold text-primary-700">수정</button></Card>
      <h2 className="mb-3 mt-7 text-[17px] font-bold">내 이사</h2><Card className="divide-y divide-line px-5"><button onClick={() => go(14)} className="flex min-h-[72px] w-full items-center text-left"><span className="grid size-10 place-items-center rounded-xl bg-canvas"><Truck size={20} /></span><span className="ml-3 flex-1"><b className="block text-[15px]">9월 12일 이사</b><small className="mt-1 block text-[12px] text-ink-400">마포구 성산동 → {destination}</small></span><ChevronRight size={18} className="text-ink-400" /></button><button onClick={() => go(12)} className="flex min-h-[72px] w-full items-center text-left"><span className="grid size-10 place-items-center rounded-xl bg-canvas"><ClipboardList size={20} /></span><span className="ml-3 flex-1"><b className="block text-[15px]">견적과 변경 기록</b><small className="mt-1 block text-[12px] text-ink-400">{phase === "quote" ? `검토 중 견적 ${quote.version}` : phase === "onsite" ? `현재 양측 수락 견적 ${quote.version}` : `현재 양측 수락 견적 ${quote.finalVersion}`}</small></span><ChevronRight size={18} className="text-ink-400" /></button></Card>
      <h2 className="mb-3 mt-7 text-[17px] font-bold">알림과 계정</h2><Card className="divide-y divide-line px-5"><div className="flex min-h-[72px] items-center"><span className="grid size-10 place-items-center rounded-xl bg-canvas"><Bell size={20} /></span><span className="ml-3 flex-1"><b className="block text-[15px]">진행 알림</b><small className="mt-1 block text-[12px] text-ink-400">견적·수정·현장 요청 알림</small></span><button role="switch" aria-checked={alerts} aria-label="진행 알림" onClick={() => setAlerts(!alerts)} className="relative grid h-11 w-14 place-items-center"><span className={`relative h-7 w-12 rounded-full ${alerts ? "bg-primary-600" : "bg-line"}`}><span className={`absolute top-1 size-5 rounded-full bg-white transition-transform ${alerts ? "left-6" : "left-1"}`} /></span></button></div><button onClick={() => setPanel("privacy")} className="flex min-h-[64px] w-full items-center text-left"><span className="flex-1 text-[15px] font-bold">개인정보 및 약관</span><ChevronRight size={18} className="text-ink-400" /></button></Card>
      <button onClick={() => setPanel("logout")} className="mt-6 min-h-11 text-[13px] font-bold text-danger-ink">로그아웃</button>
    </main><ConsumerNav active="profile" go={go} />
      <Sheet open={panel !== null} onOpenChange={(open) => !open && setPanel(null)}><SheetContent><SheetHeader><SheetTitle>{panel === "edit" ? "내 정보 수정" : panel === "privacy" ? "개인정보 및 약관" : "로그아웃할까요?"}</SheetTitle><SheetDescription>{panel === "edit" ? "이름과 연락처를 수정할 수 있어요" : panel === "privacy" ? "서비스에 적용된 보관·알림 기준이에요" : "로그아웃하면 역할 선택 화면으로 돌아가요"}</SheetDescription></SheetHeader>{panel === "edit" && <div className="space-y-4 px-6"><label className="block text-[13px] font-bold">이름<input value={draftName} onChange={(event) => setDraftName(event.target.value)} className="mt-2 h-13 w-full rounded-xl border border-line bg-white px-4 text-[15px] outline-none focus:border-primary-600" /></label><label className="block text-[13px] font-bold">연락처<input type="tel" value={draftPhone} onChange={(event) => setDraftPhone(event.target.value)} className="mt-2 h-13 w-full rounded-xl border border-line bg-white px-4 text-[15px] outline-none focus:border-primary-600" /></label></div>}{panel === "privacy" && <div className="space-y-3 px-6"><Card className="p-4"><b>영상과 사진</b><p className="mt-2 text-[13px] text-ink-600">견적과 작업 확인에만 사용하며 외부로 전송하지 않아요.</p></Card><Card className="p-4"><b>알림</b><p className="mt-2 text-[13px] text-ink-600">견적 도착, 수정 요청, 현장 변경만 알려드려요.</p></Card><Card className="p-4"><b>서비스 이용약관 · v1.0</b><p className="mt-2 text-[13px] text-ink-600">양측 수락과 변경 기록은 버전별로 보존해요.</p></Card></div>}<SheetFooter>{panel === "edit" ? <Primary disabled={!draftName.trim() || !/^010-\d{4}-\d{4}$/.test(draftPhone)} onClick={() => { setName(draftName.trim()); setPhone(draftPhone); setPanel(null); }}>저장</Primary> : panel === "logout" ? <Button variant="destructive" size="cta" className="w-full" onClick={() => window.location.assign("/")}>로그아웃</Button> : <Primary onClick={() => setPanel(null)}>확인</Primary>}</SheetFooter></SheetContent></Sheet>
    </Page>
  );
}
