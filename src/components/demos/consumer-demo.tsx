    ...
    <Page>
      ...
      <main className="flex-1 px-6 pb-6 pt-3">
        <div className="mb-6 flex items-center justify-between">
          <strong className="text-[24px] font-extrabold tracking-[-1px] text-[#4F46E5]">SEQRET</strong>
  ...
  if (demoState === "confirmation-invalidated") {
    return (
      <Page><StatusBar /><Top onBack={back} title="작업범위 v4" aside={<button onClick={() => go(12)}>지난 버전</button>} />
        <main className="flex-1 px-6 pb-5"><div className="demo-pop rounded-2xl border border-[#F5A623] bg-[#FFF6E5] p-4"><p className="text-[13px] font-bold text-[#9A6200]">기존 확인이 취소됐어요</p><p className={`mt-1 text-xs leading-5 ${muted}`}>한쪽이 v3 확인 후 작업범위를 수정해 새 v4를 만들었어요. v3의 확인은 새 버전에 자동 승계되지 않아요.</p></div><Card className="mt-4 p-5"><div className="flex justify-between"><span className={`text-xs ${muted}`}>새 제안 총액 · v4</span><Badge variant="warning">양측 재확인 필요</Badge></div><strong className="mt-2 block text-[28px]">1,280,000원</strong><p className={`mt-3 text-xs ${muted}`}>화분 2→1 · 작업 방식 메모 수정 · 금액은 동일</p></Card><h2 className="mb-3 mt-6 text-[15px] font-bold">확인 상태</h2><Card className="space-y-3 p-4"><div className="flex justify-between text-[13px]"><b>박민서 고객</b><Badge variant="warning">다시 확인 필요</Badge></div><div className="flex justify-between text-[13px]"><b>한빛이사</b><Badge variant="warning">다시 확인 필요</Badge></div><div className="border-t border-[#E9EAF2] pt-3 text-xs text-[#8E90A0]">v3 확인 기록은 감사 이력에 남지만 v4 승인에는 사용되지 않아요.</div></Card></main><Bottom><Primary onClick={() => notify("v4 소비자 확인을 기록했어요. 업체 확인이 남아 있어 아직 공동확인 완료는 아니에요.")}>이 버전 내용 확인</Primary><Outline onClick={() => go(11)}>다시 수정 요청</Outline></Bottom>
      </Page>
    );
  }
  return (
  ...
  return (
    <Page>
      ...
      <main className="flex-1 px-5 pb-5">
        <h1 className="mt-2 text-[24px] font-extrabold leading-[32px]">같은 공간을<br />전·후로 확인해요</h1>
        <p className={`mt-2 text-[13px] ${muted}`}>완료 확인 뒤에도 촬영 시각과 원본 기록은 그대로 보존돼요.</p>
        <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
          {rooms.map(([label], index) => (
            <button
              className={`h-10 shrink-0 rounded-full px-4 text-[12px] font-bold ${roomIndex === index ? "bg-[#191927] text-white" : "bg-white text-[#4B4B5C]"}`}
              key={label}
        ...
        <Card className="mt-4 p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            {[["작업 전", beforeAt], ["완료", afterAt]].map(([label, time]) => (
              <button className="demo-interactive-card overflow-hidden rounded-2xl border border-[#E9EAF2] bg-white text-left" key={label} type="button">
                <span className="grid h-32 place-items-center bg-[#E5E6EE] text-[#747785]"><Camera size={28} /></span>
...
export function ConsumerDemo() {
  ...
  useEffect(() => {
    ...
    if (!Number.isInteger(parsed) || parsed < 1 || parsed > 13) return;
    const timer = window.setTimeout(() => setScreen(parsed), 0);
    return () => window.clearTimeout(timer);
  }, [requestedScreen]);
  const go = (target: number) => setScreen(Math.min(13, Math.max(1, target)));
  const nextMap: Record<number, number> = { 1: 2, 2: 3, 3: 8, 8: 4, 4: 10, 9: 4, 10: 5, 11: 5, 5: 6, 6: 7, 7: 13, 13: 12, 12: 1 };
  const backMap: Record<number, number> = { 2: 1, 3: 2, 8: 3, 4: 8, 9: 4, 10: 4, 5: 10, 11: 5, 6: 5, 7: 6, 13: 7, 12: 13 };
