    ...
    <div className="min-h-dvh bg-canvas text-ink-900 lg:grid lg:grid-cols-[232px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-[var(--z-sticky)] flex min-h-[var(--header-height)] items-center justify-between gap-4 border-b border-line bg-surface/96 px-6 backdrop-blur xl:px-10">
          <div className="min-w-0"><p className="text-ui-control text-ink-600">{view === "jobs" ? jobMode === "quote" ? "작업 관리 / 견적 작성" : "작업 운영" : view === "issues" ? "현장 변경 관리" : "업체 운영"}</p><h2 className="mt-1 truncate text-ui-section">{view === "jobs" ? "작업 관리" : view === "issues" ? "현장 이슈" : view === "invite" ? "기사 초대" : "설정"}</h2></div>
          <div className="flex items-center gap-2">
            <label className="hidden h-[var(--control-touch)] min-w-[280px] items-center gap-2 rounded-[var(--radius-control)] border border-line bg-surface px-4 focus-within:border-primary-400 focus-within:ring-3 focus-within:ring-primary-100 md:flex"><Search aria-hidden="true" className="text-ink-600" /><input aria-label="고객명, 주소, 작업 ID 검색" className="min-w-0 flex-1 bg-transparent text-sm outline-none" data-slot="input-proxy" onChange={(event) => setSearch(event.target.value)} placeholder="고객명, 주소, 작업 ID 검색" value={search} /></label>
