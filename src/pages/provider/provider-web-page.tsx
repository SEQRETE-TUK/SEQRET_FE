...
// 업체 다중 연결 목록은 accessToken을 포함하므로 메모리에만 보관한다.
// docs/api-spec.md: 민감 응답을 PWA cache, localStorage, sessionStorage, IndexedDB, log와 analytics에 남기지 않는다.
    ...
    <div className="min-h-dvh bg-canvas text-ink-900 lg:grid lg:grid-cols-[232px_minmax(0,1fr)]">
      <aside className="hidden min-h-dvh border-r border-line bg-surface px-4 py-8 lg:flex lg:flex-col">
        ...
        <div className="mt-auto rounded-xl border border-line p-3">
          <p className="text-xs font-bold text-ink-600">연결된 이사</p><p className="mt-1 truncate text-sm font-extrabold text-ink-900">{session ? session.actor.display_name : "연결된 이사 없음"}</p>          {session ? <Button className="mt-3 w-full justify-start" onClick={onDisconnect} size="chip" variant="ghost"><SignOut aria-hidden="true" /> 이사 연결 해제</Button> : <Button className="mt-3 w-full justify-start" onClick={() => setConnectionOpen(true)} size="chip" variant="ghost"><Key aria-hidden="true" /> 초대 코드 입력</Button>}
        </div>
      </aside>
      ...
      <div className="min-w-0">
        ...
        <main className="min-w-0 bg-surface-muted p-5 xl:p-8">
          ...
          {view === "invite" ? session ? <OperationsPanel onOpenIssues={() => changeView("issues")} /> : <ProviderConnectionEmpty onConnect={() => setConnectionOpen(true)} /> : null}
...
// 상단 4칸 카드가 곧 필터 탭이다. 뎁스가 얕은 화면이라 상태를 색으로 구분한다.
type JobFilter = "all" | "review" | "customer" | "confirmed";

function jobStatusKey(status: string): Exclude<JobFilter, "all"> {
  if (status === "customer_review") return "customer";
  if (status === "confirmed") return "confirmed";
  return "review";
}

const statusMeta = {
  review: { label: "검토 필요", tag: "primary", rail: "bg-primary-600", chip: "bg-primary-50 text-primary-700", card: "border-primary-600 bg-primary-50" },
  customer: { label: "고객 확인 중", tag: "warning", rail: "bg-warning-ink", chip: "bg-warning-bg text-warning-ink", card: "border-warning-ink bg-warning-bg" },
  confirmed: { label: "공동 확정", tag: "success", rail: "bg-success", chip: "bg-success-bg text-success-ink", card: "border-success bg-success-bg" },
} as const;

function JobDashboard({ jobs: linkedJobs, onConnect, onQuote, onSelect, onShowOperations, scope, search, selectedJobId, showOperations }: { jobs: Array<{ session: AuthSession; scope: ScopeReview | undefined }>; onConnect: () => void; onQuote: () => void; onSelect: (session: AuthSession) => void; onShowOperations: () => void; scope: ScopeReview | undefined; search: string; selectedJobId: string | null; showOperations: boolean }) {
  ...
  const stats = [
    ["review", "검토 필요", loadedScopes.filter((item) => item.scope.status === "company_review" || item.scope.status === "revision_requested").length, "primary"],
    ["customer", "고객 확인 중", loadedScopes.filter((item) => item.scope.status === "customer_review").length, "warning"],
    ["confirmed", "공동 확정", loadedScopes.filter((item) => item.scope.status === "confirmed").length, "success"],
  ] as const;
  const visibleJobs = filter === "all" ? jobs : jobs.filter((job) => jobStatusKey(job.status) === filter);
  return <>
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{stats.map(([key, label, value, tone]) => {
      const active = filter === key;
      const meta = statusMeta[key];
      return <button
        aria-pressed={active}
        className={`ui-card flex min-h-24 items-center gap-4 border-2 p-5 text-left transition ${active ? meta.card : "border-transparent hover:border-line"}`}
        key={key}
        onClick={() => setFilter(active ? "all" : key)}
        type="button"
      >
        <span className={`grid size-11 place-items-center rounded-xl ${meta.chip}`}>{tone === "success" ? <CheckCircle aria-hidden="true" /> : <Clipboard aria-hidden="true" />}</span>
        <div><p className="text-sm text-ink-600">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div>
        <span className={`ml-auto h-10 w-1.5 rounded-full ${meta.rail}`} />
      </button>;
    })}</section>
    <section className="mt-5 grid min-h-[690px] gap-5 xl:grid-cols-[minmax(360px,0.9fr)_minmax(440px,1.1fr)]">
      <div className="ui-card overflow-hidden"><div className="flex min-h-14 items-center justify-between gap-3 border-b border-line px-5 text-sm font-extrabold"><span>{filter === "all" ? "전체" : statusMeta[filter].label}</span><span className="text-xs font-bold text-ink-600">{visibleJobs.length}건</span></div><div className="p-4"><p className="mb-3 text-xs font-bold text-ink-600">최신순 <CaretDown aria-hidden="true" className="inline" /></p>{visibleJobs.length ? visibleJobs.map((job) => {
        const meta = statusMeta[jobStatusKey(job.status)];
        return <button className={`flex w-full items-stretch gap-3 rounded-xl border px-3 py-4 text-left ${job.current ? "border-primary-600 bg-primary-50/50" : "border-transparent hover:bg-surface-muted"}`} key={job.session.actor.job_id} onClick={() => onSelect(job.session)} type="button"><span className={`w-1.5 shrink-0 rounded-full ${meta.rail}`} /><time className="w-12 shrink-0 self-center text-center text-sm font-extrabold">{job.date ? day.format(new Date(job.date)) : "–"}</time><div className="min-w-0 flex-1"><p className="font-extrabold">{job.customer} <StatusTag tone={meta.tag}>{meta.label}</StatusTag></p><p className="mt-1 truncate text-sm text-ink-600">{job.route}</p><p className="mt-1 text-xs text-ink-400">작업 ID {job.jobCode}</p></div><CaretRight aria-hidden="true" className="self-center" /></button>;
      }) : jobs.length ? <p className="px-3 py-10 text-center text-sm text-ink-600">{statusMeta[filter === "all" ? "review" : filter].label} 상태인 작업이 없어요.</p> : <ProviderConnectionEmpty onConnect={onConnect} />}</div></div>
...
function IssueWorkbench({ connection, issues, scope }: { connection: Connection; issues: FieldIssue[]; scope: ScopeReview | undefined }) {
  ...
  return <div className="grid min-h-[760px] gap-5 xl:grid-cols-[320px_minmax(0,1fr)] 2xl:grid-cols-[380px_minmax(0,1fr)]"><section className="ui-card overflow-hidden"><div className="flex min-h-16 items-center gap-6 border-b border-line px-5 text-sm font-extrabold"><span className="border-b-2 border-primary-600 py-5">전체 {issues.length}</span><span className="text-ink-600">처리 필요 {issues.filter((issue) => issue.status === "open").length}</span></div><div className="space-y-2 p-4">{issues.map((issue) => { const tone = issueTone(issue.status); return <button className={`flex w-full items-stretch gap-3 rounded-xl border p-4 text-left ${selected?.field_issue_id === issue.field_issue_id ? "border-primary-600 bg-primary-50/60" : "border-line hover:bg-surface-muted"}`} key={issue.field_issue_id} onClick={() => setSelectedId(issue.field_issue_id)} type="button"><span className={`w-1.5 shrink-0 rounded-full ${tone.rail}`} /><div className="min-w-0 flex-1"><p className="font-extrabold">{issue.title}</p><p className="mt-1 text-sm text-ink-600">{issueTypeLabel(issue.issue_type)} · {scope?.job.customer_display_name ?? "고객"}</p></div><StatusTag tone={tone.tag}>{tone.label}</StatusTag></button>; })}</div><div className="border-t border-line p-4"><p className="text-xs text-ink-600">현장 이슈는 현장에서 기사가 증거와 함께 보고합니다. 업체는 보고된 이슈를 변경 제안으로 전환합니다.</p></div></section><section className="ui-card ui-card-pad">{selected && scope ? <><div className="grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"><div className="rounded-xl border border-line bg-surface-muted p-4"><div className="flex items-center justify-between"><h3 className="text-lg font-black">기준 승인본</h3><StatusTag tone="neutral">{scope.scope.version_label}</StatusTag></div><p className="mt-1 text-xs text-ink-600">고객이 승인한 내용입니다. 여기서는 바뀌지 않아요.</p><p className="mt-4 text-sm font-extrabold">현장 증거</p><img alt={selected.title} className="mt-2 aspect-[16/10] w-full rounded-xl object-cover" src="/elevator-outage-evidence.png" /><p className="mt-3 text-sm font-extrabold">기사 설명</p><p className="mt-2 rounded-xl border border-line bg-surface p-4 text-sm text-ink-600">{selected.description}</p><h4 className="mt-5 font-extrabold">관련 승인 범위</h4><div className="mt-2 rounded-xl border border-line bg-surface p-4 text-sm"><p>포장 및 운반 <span className="float-right">포함</span></p><p className="mt-2">기본 가구 분해·조립 <span className="float-right">포함</span></p><p className="mt-2">에어컨 탈부착 <span className="float-right">제외</span></p></div></div><div className="rounded-xl border-2 border-primary-600 bg-surface p-4"><div className="flex items-center justify-between"><h3 className="flex items-center gap-2 text-lg font-black text-primary-700"><Wrench aria-hidden="true" /> 변경 제안</h3><StatusTag tone="primary">작성 중</StatusTag></div><p className="mt-1 text-xs text-ink-600">고객이 승인하기 전까지 {scope.scope.version_label}이 유지됩니다.</p><p className="mt-4 text-sm font-extrabold">변경 제안 내용</p><div className="mt-3 rounded-xl border border-line p-4"><p className="text-sm text-ink-600">추가 작업</p><p className="mt-2 font-extrabold">계단 운반 · 5층 <span className="float-right">1건</span></p></div><label className="mt-5 block text-sm font-extrabold" htmlFor="change-reason">사유</label><Textarea className="mt-2" id="change-reason" onChange={(event) => setReason(event.target.value)} value={reason} /><div className="mt-5 space-y-3 text-sm"><p className="flex justify-between"><span>기존 금액 ({scope.scope.version_label})</span><strong>{money(scope.quote?.total_amount_krw)}</strong></p><MoneyInput label="추가/조정 금액" onChange={setAdjustment} value={adjustment} /><p className="flex items-center justify-between border-t border-line pt-4"><span className="font-extrabold">변경 후 금액</span><strong className="text-2xl text-primary-700">{money((scope.quote?.total_amount_krw ?? 0) + adjustment)}</strong></p></div>{mutation.error ? <p className="mt-3 text-sm font-bold text-danger">{apiErrorMessage(mutation.error)}</p> : null}<Button className="mt-5 w-full" disabled={mutation.isPending || selected.status !== "open"} onClick={() => mutation.mutate()} size="cta"><Send aria-hidden="true" /> 변경 제안 보내기</Button><Button className="mt-3 w-full" variant="outline">설명 요청</Button></div></div></> : <p className="text-sm text-ink-600">처리할 현장 이슈가 없습니다.</p>}</section></div>;
}

// 현장 이슈 상태를 색으로 구분한다. 화면 뎁스가 얕아 상태는 색으로만 읽힌다.
function issueTone(status: FieldIssue["status"]): { label: string; rail: string; tag: "primary" | "success" | "warning" | "neutral" } {
  if (status === "open") return { label: "처리 필요", rail: "bg-danger", tag: "warning" };
  if (status === "customer_review") return { label: "고객 확인 중", rail: "bg-warning-ink", tag: "warning" };
  if (status === "clarification_requested") return { label: "설명 요청됨", rail: "bg-primary-600", tag: "primary" };
  if (status === "approved") return { label: "승인 완료", rail: "bg-success", tag: "success" };
  return { label: "거절됨", rail: "bg-ink-400", tag: "neutral" };
}

function issueTypeLabel(type: FieldIssue["issue_type"]): string {
  if (type === "out_of_scope") return "범위 밖 작업";
  if (type === "damage_risk") return "파손 위험";
  return "현장 장애";
}

function OperationsPanel({ onOpenIssues }: { onOpenIssues: () => void }) { return <section className="ui-card mx-auto max-w-5xl p-6"><h3 className="text-2xl font-black">기사 초대와 배차</h3><div className="mt-5"><LiveProviderWorkflow embedded onOpenIssues={onOpenIssues} wide /></div></section>; }
