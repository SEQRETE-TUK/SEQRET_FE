...
function Top({ onBack, title, aside }: { onBack: () => void; title?: string; aside?: ReactNode }) {
  return (
    <div className="sticky top-0 z-20 flex h-14 items-center justify-between bg-canvas/95 px-5 backdrop-blur-sm">
      ...
      {title && <p className="text-ui-section">{title}</p>}
  ...
  return (
    <Page>
      ...
      <main className="flex-1 px-6 pb-28 pt-3">
        ...
        <h1 className="mb-4 whitespace-nowrap text-ui-section font-extrabold leading-9">{task.headline}</h1>
...
function MoveHub({ go, phase, company, destination, destinationDetail }: { go: (screen: number) => void; phase: MovePhase; company: string; destination: string; destinationDetail: string }) {
  ...
  return (
    <Page>
      <StatusBar />
      <header className="flex h-14 items-center px-6"><h1 className="text-ui-section font-extrabold">내 이사</h1></header>
      <main className="flex-1 px-6 pb-28 pt-3">
        <p className="text-base font-semibold text-ink-400">9월 12일 토요일 · 오전 8시</p>
        <h2 className="mt-1 text-ui-section font-extrabold">출발지와 목적지</h2>
  ...
  return (
    <Page>
      ...
      <main className="flex-1 px-6 pb-5 pt-5">
        <h1 className="text-ui-section font-extrabold">도착지 조건을<br />알려주세요</h1><p className={`mt-1 text-base ${muted}`}>모르면 ‘모름’을 선택해도 돼요 — 업체가 확인해 드려요</p>
...
function Capture({ next, back }: { next: () => void; back: () => void }) {
  ...
  return (
    <Page><StatusBar /><div className="flex items-center justify-between px-5"><BackButton onClick={back} /><Dots current={3} /><span className={`text-base font-bold ${muted}`}>3/4</span></div>
      <main className="flex-1 px-6 pb-4 pt-5"><h1 className="text-ui-section font-extrabold">구역마다 한 번씩<br />천천히 찍어주세요</h1><p className={`mt-1 text-base ${muted}`}>15~30초면 충분해요 · 얼굴·귀중품은 피해주세요</p>
...
function ScopeSummary({ next, back, go, company, accepted, revisionRequested, demoState = "" }: { next: () => void; back: () => void; go: (screen: number) => void; company: string; accepted: boolean; revisionRequested: boolean; demoState?: string }) {
  ...
  return (
    <Page><StatusBar /><Top onBack={back} title={`${quote.name} 견적 ${quote.version}`} aside={<button className="font-bold text-primary-700" onClick={() => go(12)}>이전 견적 비교</button>} /><main className="flex-1 px-6 pb-5"><h1 className="mb-5 mt-3 whitespace-nowrap text-ui-section font-extrabold">{revisionRequested ? "수정 견적을 기다리고 있어요" : "이 견적으로 진행할까요?"}</h1><Card className="p-5"><div className="flex items-center justify-between"><span className={`text-lg font-semibold ${muted}`}>제안 총액</span><span className={`text-base font-bold ${confirmed ? "text-success-ink" : "text-warning-ink"}`}>{confirmed ? "수락 완료" : revisionRequested ? "업체 답변 대기" : "내 수락 대기"}</span></div><p className="mt-2 text-ui-section font-extrabold">{quote.price}</p><div className="mt-4 flex items-center justify-between border-t border-line pt-4"><span className="text-base text-ink-600">AI 예상 견적 대비</span><strong className="text-xl text-danger-ink">{quote.delta}</strong></div><p className={`mt-4 text-lg leading-6 ${muted}`}>{quote.detail} · 예상 6시간</p></Card>
  ...
  return (
    <Page>
      ...
      <main className="flex-1 px-5 pb-5">
        <h1 className="mt-2 text-ui-section font-extrabold">같은 공간을<br />전·후로 확인해요</h1>
...
function Invite({ next, back, selected, onSelect }: { next: () => void; back: () => void; selected: string; onSelect: (company: string) => void }) {
  return (
    <Page>
      ...
      <main className="flex-1 px-6 pb-5">
        <h1 className="mt-4 whitespace-nowrap text-ui-section font-extrabold">3개 업체가 견적을 보냈어요</h1>
...
function Profile({ go, phase, company, destination }: { go: (screen: number) => void; phase: MovePhase; company: string; destination: string }) {
  ...
  return (
    <Page><StatusBar /><header className="flex h-14 items-center px-6"><h1 className="text-ui-section font-extrabold">내 정보</h1></header><main className="flex-1 px-6 pb-28 pt-3">
