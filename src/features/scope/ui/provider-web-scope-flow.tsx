...
function FieldIssueView({ demoState = "" }: { demoState?: string }) {
  ...
  return <>
    <div className="flex items-start justify-between gap-4">
      ...
      <Link className="rounded-xl border border-line bg-white px-4 py-3 text-[13px] font-bold" to="/crew?screen=3">작업자 보고 보기</Link>
    </div>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(340px,1fr)]">
      ...
      <div className="space-y-5">
        <Card className="border-primary-400 p-5"><h2 className="text-[17px] font-bold">변경 편집</h2><div className="mt-5 flex justify-between text-[13px]"><span className={muted}>기존 금액</span><b>1,280,000원</b></div><label className="mt-4 block text-[13px] font-bold">증감 금액<Input name="amountDelta" className="mt-2 h-12 w-full rounded-xl bg-canvas px-4 text-right text-[16px] font-bold outline-none" disabled={sent} min="0" onChange={(event) => setDelta(event.target.value)} type="number" value={delta} /></label><label className="mt-4 block text-[13px] font-bold">변경 사유<Textarea name="changeReason" className="mt-2 h-24 w-full resize-none rounded-xl bg-canvas p-4 text-[13px] outline-none" defaultValue="엘리베이터 고장으로 사다리차 하차 방법이 필요합니다." disabled={sent} /></label><div className="my-5 h-px bg-line" /><div className="flex items-end justify-between"><b>변경 후 총액</b><strong className="text-2xl text-primary-800">{total.toLocaleString("ko-KR")}원</strong></div><Button className="mt-6 w-full" disabled={blocked || sent || sending || !Number(delta)} onClick={() => { setSending(true); window.setTimeout(() => { setSending(false); setSent(true); notify("변경안 v4를 고객에게 보냈어요."); }, 500); }} size="cta">{sending ? <><LoaderCircle className="demo-spin" size={18} /> 전송 중...</> : sent ? "고객 전송 완료" : blocked ? "문제를 먼저 해결해 주세요" : "변경안 고객에게 보내기"}</Button></Card>
...
function AssignView({ next, demoState = "" }: { next: () => void; demoState?: string }) {
  ...
  const notify = useDemoFeedback();
  return <><Badge variant="warning">피아노 작업 포함</Badge><h1 className="mt-4 text-[26px] font-extrabold leading-[34px]">배차 · 인력 배정</h1><p className={`mt-1 text-[13px] ${muted}`}>확정 범위 v3 · 5톤 1대 · 작업자 4명 · 예상 6시간</p><div className="mt-5 grid gap-5 xl:grid-cols-2"><div className="space-y-5"><Card className="p-5"><div className="flex justify-between"><b>차량 선택</b><Badge>후보 2대</Badge></div>{[['12가3456 · 5톤 리프트 · 적재함 28㎡','당일 일정 없음'],['34나7890 · 5톤 · 적재함 26㎡','09:30 다른 일정과 충돌']].map(([name,sub],i) => <button className={`demo-interactive-card mt-3 flex w-full items-center rounded-xl border p-4 text-left ${vehicle === i ? (i ? 'border-warning bg-warning-bg' : 'border-primary-400 bg-primary-50') : 'border-line'}`} key={name} onClick={() => setVehicle(i)} type="button"><span className={`mr-3 size-5 rounded-full border-2 ${vehicle === i ? `border-[6px] ${i ? 'border-warning' : 'border-primary-800'}` : 'border-[var(--color-rule-2)]'}`} /><div><b>{name}</b><p className={`text-xs ${i ? 'text-warning' : muted}`}>{sub}</p></div></button>)}</Card><Card className="p-5"><div className="flex justify-between"><b>작업자 배정</b><Badge variant={externalConflictLabel ? "warning" : "success"}>4 / 4명</Badge></div>{crew.map(([name,skill], index) => <div className="flex h-12 items-center border-b border-line text-[13px] last:border-0" key={name}><b className="w-52">{name}</b><span className={`flex-1 ${muted}`}>{skill}</span>{externalConflictLabel && index === 0 ? <Badge variant="warning">확인 필요</Badge> : <Button onClick={() => notify(`${name}의 일정·자격 상세를 확인했어요.`)} size="chip">배정됨</Button>}</div>)}</Card></div><div className="space-y-5"><Card className={`p-5 ${conflict ? 'border-warning bg-warning-bg' : 'border-success bg-success-bg'}`}><h2 className="font-bold">충돌 확인 (7종 자동 검사)</h2>{conflict ? <div className="mt-5 space-y-3 text-[13px] font-bold text-warning-ink"><p>! {conflictLabel}</p><p>✓ 차량 용량 · 필요 24㎡ / 후보 {vehicle === 1 ? "26" : "28"}㎡</p><p>✓ 인원 충족 · 4/4</p><p className="text-danger-ink">문제 항목을 해결하기 전에는 배정을 확정할 수 없어요.</p></div> : <div className="mt-5 grid grid-cols-2 gap-5 text-[13px] font-bold text-success-ink">{['차량 일정 · 충돌 없음','차량 용량 · 여유 4㎡','작업자 일정 · 충돌 없음','인원 충족 · 4/4','피아노 자격 · 2명 유효','근무시간 · 예상 7.5h'].map(x => <span key={x}>✓ {x}</span>)}</div>}</Card><Card className="p-5"><b>작업자 전달 메모</b><Textarea name="crewNote" className="mt-4 h-20 w-full resize-none rounded-xl border border-line bg-canvas p-4 text-[13px] outline-none" defaultValue="피아노 이동 전 바닥 보강 · 도착지 엘리베이터 상태 우선 확인\n고객 연락은 팀장만 · 동호수는 현장에서 안내" /></Card><Button disabled={conflict || assigning} className="w-full" onClick={() => { if (conflict || assigning) return; setAssigning(true); window.setTimeout(next, 500); }} size="cta">{conflict ? '충돌을 먼저 해결해 주세요' : assigning ? <><LoaderCircle className="demo-spin" size={18} /> 배정 확정 중...</> : '배정 확정 · 작업자 링크 발송'}</Button></div></div></>;
}
