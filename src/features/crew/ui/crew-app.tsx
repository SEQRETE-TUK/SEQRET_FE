...
function CrewInviteHero({ onInvite }: { onInvite: () => void }) {
  return <button className="mt-4 ui-card ui-card-outlined ui-card-tinted press-static relative min-h-[174px] w-full overflow-hidden rounded-[var(--radius-component)] px-5 py-4 text-left" onClick={onInvite} type="button"><span className="relative z-10 block max-w-[62%]"><strong className="block text-ui-component font-extrabold tracking-[var(--tracking-display)]">연결 코드로<br />오늘 작업 연결하기</strong><span className="mt-1.5 block text-ui-data leading-5 text-ink-600">배정된 이사와 현장 정보를 한 번에 불러와요.</span><span className="mt-4 flex items-center gap-1 whitespace-nowrap text-ui-control text-primary-700">연결 코드 입력하기 <CaretRight aria-hidden="true" size="var(--icon-xs)" /></span></span><img alt="휴대폰으로 이삿짐을 확인하는 모습" className="absolute -right-2 bottom-0 w-[140px] max-w-none" height="213" src="/move-capture-hero.png" width="170" /></button>;
}
...
function CrewWorkList({ moveJobs, onNewWork, onOpen, onSelect }: { moveJobs: MockMoveSummary[] | undefined; onNewWork: () => void; onOpen: () => void; onSelect: (jobId: string) => void }) {
  ...
  return <div className="pb-28">
    <div className="bg-surface px-[var(--content-gutter)] pt-5"><div className="flex items-center justify-between gap-3"><h1 className="text-ui-section">내 작업</h1><button className="inline-flex min-h-11 items-center gap-1 whitespace-nowrap px-2 text-ui-control text-primary-700" onClick={onNewWork} type="button"><PlusCircle aria-hidden="true" size="var(--icon-xs)" weight="bold" />새 작업</button></div><div className="mt-6 grid grid-cols-2 border-b border-line" role="tablist" aria-label="기사 작업 목록"><button aria-selected={listTab === "active"} className={`relative min-h-11 text-ui-control ${listTab === "active" ? "text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" : "text-ink-600"}`} onClick={() => setListTab("active")} role="tab" type="button">진행 중 {activeMoves.length}</button><button aria-selected={listTab === "history"} className={`relative min-h-11 text-ui-control ${listTab === "history" ? "text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" : "text-ink-600"}`} onClick={() => setListTab("history")} role="tab" type="button">작업 기록 {historyMoves.length}</button></div></div>
...
function CrewHome({ brief, displayName, issueCount, onInvite, onWork }: {
  ...
  return (
    <div className="px-[var(--content-gutter)] pb-28 pt-4">
      ...
      {brief ? <><ActiveMoveCard heading="진행 중인 이사 1건" leading={<span className="grid size-12 shrink-0 place-items-center rounded-full bg-primary-50 text-primary-700"><Truck aria-hidden="true" size="var(--icon-md)" /></span>} meta={startAt ? `${dayFormatter.format(startAt)} · ${timeFormatter.format(startAt)}` : "일정 확인 중"} onOpen={onWork} route={<>{brief.masked_origin ?? "출발지"} → {brief.masked_destination ?? "도착지"}</>}><MoveJourneyProgress current={brief.completion_submission_id ? 4 : brief.checked_in_at ? 3 : 2} /><div className="mt-3 border-t border-line pt-3"><p className="flex min-w-0 items-baseline gap-2 whitespace-nowrap"><strong className="shrink-0 text-ui-support text-primary-700">작업범위 확인</strong><span className="ml-auto min-w-0 truncate text-right text-ui-data text-ink-600">최신 승인본과 현장 조건을 확인해요</span></p><button className="mt-2 flex min-h-10 w-full items-center justify-center rounded-xl bg-primary-600 text-ui-support font-extrabold text-white" onClick={onWork} type="button">지금 확인</button></div></ActiveMoveCard><aside className="mt-4 flex items-start gap-3 rounded-[var(--radius-component)] bg-warning-bg p-3 text-sm font-medium leading-5 text-warning-ink"><WarningCircle aria-hidden="true" className="mt-0.5 shrink-0" /><p>현장이 승인본과 다르면 추가 금액을 요구하지 말고 먼저 보고해 주세요. {issueCount > 0 ? `현재 보고 ${issueCount}건이 처리 중이에요.` : ""}</p></aside></> : <section className="mt-4 ui-card px-5 py-8 text-center"><ClockCounterClockwise aria-hidden="true" className="mx-auto text-ink-400" size="var(--icon-category)" /><h2 className="mt-3 text-ui-component">업체의 작업 확정을 기다리고 있어요</h2><p className="mt-2 text-ui-support text-ink-600">배차와 작업범위가 확정되면 오늘 작업이 표시됩니다.</p></section>}
    </div>
  );
}
