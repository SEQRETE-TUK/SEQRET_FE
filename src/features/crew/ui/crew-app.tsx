...
function CrewGuestHome({ onInvite }: { onInvite: () => void }) {
  return <div className="px-[var(--content-gutter)] pb-28 pt-4"><h1 className="text-ui-section leading-9 font-black tracking-[var(--tracking-display)]">배정된 이사를<br />연결해 주세요</h1><CrewInviteHero onInvite={onInvite} /><section className="mt-4 rounded-[var(--radius-card)] border border-line bg-surface px-5 py-8 text-center"><Archive aria-hidden="true" className="mx-auto text-ink-400" size="var(--icon-category)" /><h2 className="mt-3 font-black">연결된 작업이 없어요</h2></section></div>;
}

function CrewInviteHero({ onInvite }: { onInvite: () => void }) {
  return <button className="press-static relative mt-5 min-h-[168px] w-full overflow-hidden rounded-[var(--radius-feature)] border border-primary-100 bg-primary-50 p-5 text-left" onClick={onInvite} type="button"><span className="relative z-10 block max-w-[62%]"><strong className="block text-ui-section font-black">초대 코드로<br />오늘 작업 연결하기</strong><span className="mt-2 block text-sm leading-5 text-ink-600">배정된 이사와 현장 정보를 한 번에 불러와요.</span><span className="mt-4 flex items-center gap-1 text-sm font-extrabold text-primary-700">초대 코드 입력하기 <CaretRight aria-hidden="true" size="var(--icon-sm)" /></span></span><img alt="휴대폰으로 이삿짐을 확인하는 모습" className="absolute -right-7 -bottom-4 w-[176px] max-w-none" height="220" src="/move-capture-hero.png" width="176" /></button>;
}

function CrewHomeHeader({ onBell }: { onBell: () => void }) {
  return <header className="app-safe-header relative z-[var(--z-sticky)] h-0 bg-canvas"><button aria-label="알림 확인" className="absolute top-[max(8px,env(safe-area-inset-top))] right-[var(--content-gutter)] grid size-9 place-items-center rounded-full text-ink-900 hover:bg-surface-muted" onClick={onBell} type="button"><Bell aria-hidden="true" size="var(--icon-md)" /><span aria-hidden="true" className="absolute top-1 right-1 size-2 rounded-full bg-danger" /></button></header>;
}

function CrewNotificationsHeader({ onBack }: { onBack: () => void }) {
  return <header className="app-safe-header sticky top-0 z-[var(--z-sticky)] grid min-h-[64px] grid-cols-[44px_1fr_44px] items-center border-b border-line bg-surface px-2"><button aria-label="홈으로 돌아가기" className="grid size-9 place-items-center rounded-full" onClick={onBack} type="button"><CaretLeft aria-hidden="true" size="var(--icon-md)" weight="bold" /></button><h1 className="text-center text-ui-section font-black">알림</h1><span /></header>;
}

function CrewNotifications({ onAction }: { onAction: () => void }) {
  return <div className="px-[var(--content-gutter)] pb-28 pt-5"><h2 className="text-ui-section font-black tracking-[var(--tracking-display)]">새로 도착한 소식</h2><p className="mt-2 text-sm text-ink-600">작업 변경과 고객 요청을 모아서 보여드려요.</p><article className="mt-6 border-y border-line py-5"><div className="flex gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-[var(--radius-card)] bg-primary-50 text-primary-700"><ChatCircleDots aria-hidden="true" size="var(--icon-md)" /></span><div className="min-w-0"><span className="text-xs font-extrabold text-primary-700">고객 수정 요청</span><h3 className="mt-1 text-ui-component font-black">변경안을 다시 확인해 주세요</h3><p className="mt-1 text-sm leading-5 text-ink-600">추가 시간과 운반 인원을 다시 확인해 달라는 요청이 도착했어요.</p><p className="mt-2 text-xs text-ink-400">방금 전 · 성수동 1가 이사</p></div></div><button className="mt-4 flex min-h-11 w-full items-center justify-between border-t border-line pt-3 text-sm font-extrabold text-primary-700" onClick={onAction} type="button">요청 내용 확인 <CaretRight aria-hidden="true" size="var(--icon-sm)" /></button></article></div>;
}
...
function CrewWorkList({ brief, onOpen }: { brief: Awaited<ReturnType<typeof getFieldBrief>> | undefined; onOpen: () => void }) {
  ...
  return <div className="px-[var(--content-gutter)] pb-28 pt-6">
    <h1 className="text-ui-section font-black tracking-[var(--tracking-display)]">내 작업</h1>
    <div className="mt-6 grid grid-cols-2 border-b border-line" role="tablist" aria-label="기사 작업 목록"><button aria-selected={listTab === "active"} className={`relative min-h-11 text-sm font-semibold ${listTab === "active" ? "text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" : "text-ink-600"}`} onClick={() => setListTab("active")} role="tab" type="button">진행 중 {hasWork && !completed ? 1 : 0}</button><button aria-selected={listTab === "history"} className={`relative min-h-11 text-sm font-semibold ${listTab === "history" ? "text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" : "text-ink-600"}`} onClick={() => setListTab("history")} role="tab" type="button">작업 기록 {historyCount}</button></div>
    {showWork ? <button className="press-static mt-5 w-full rounded-[var(--radius-feature)] border border-line bg-surface p-5 text-left shadow-[var(--shadow-card)]" onClick={onOpen} type="button"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-extrabold ${completed ? "bg-success-bg text-success-ink" : "bg-primary-50 text-primary-700"}`}>{completed ? "작업 완료" : brief?.checked_in_at ? "현장 진행" : "작업 예정"}</span><strong className="mt-4 flex items-center justify-between gap-3 text-ui-section"><span className="min-w-0 truncate">{brief ? `${brief.masked_origin ?? "출발지"} → ${brief.masked_destination ?? "도착지"}` : "작업 정보를 불러오는 중"}</span><CaretRight aria-hidden="true" className="shrink-0 text-ink-400" size="var(--icon-sm)" /></strong><span className="mt-2 block text-sm text-ink-600">{startAt ? `${dayFormatter.format(startAt)} · ${timeFormatter.format(startAt)}` : "일정 확인 중"}</span><span className="mt-5 grid grid-cols-3 divide-x divide-line border-t border-line pt-4 text-center text-xs text-ink-600"><span><Package aria-hidden="true" className="mx-auto mb-1" size="var(--icon-sm)" />점검 {brief?.completion_required_count ?? "–"}개</span><span><Wrench aria-hidden="true" className="mx-auto mb-1" size="var(--icon-sm)" />작업 {brief?.completion_check_items.length ?? "–"}개</span><span><Buildings aria-hidden="true" className="mx-auto mb-1" size="var(--icon-sm)" />조건 {brief?.origin_conditions.length ?? "–"}개</span></span></button> : listTab === "history" && mockApiEnabled && hasWork ? <div className="mt-5 space-y-3"><CrewHistoryCard date="2026년 5월 18일" destination="합정동" origin="성수동" reports={1} /><CrewHistoryCard date="2025년 8월 25일" destination="성수동" origin="건대입구" reports={0} /></div> : <section className="mt-5 rounded-[var(--radius-feature)] border border-line bg-surface px-5 py-8 text-center"><Archive aria-hidden="true" className="mx-auto text-ink-400" size="var(--icon-category)" /><h2 className="mt-3 font-black">{listTab === "active" ? "진행 중인 작업이 없어요" : "완료한 작업이 없어요"}</h2></section>}
  </div>;
}

function CrewHistoryCard({ date, destination, origin, reports }: { date: string; destination: string; origin: string; reports: number }) {
  return <article className="rounded-[var(--radius-feature)] border border-line bg-surface p-5"><span className="inline-flex rounded-full bg-success-bg px-3 py-1 text-xs font-extrabold text-success-ink"><Check aria-hidden="true" className="mr-1" size="var(--icon-xs)" />완료</span><h2 className="mt-4 text-ui-section font-black">{origin} → {destination}</h2><p className="mt-2 text-sm text-ink-600">{date}</p><div className="mt-4 flex gap-4 border-t border-line pt-4 text-sm text-ink-600"><span>완료 체크 4/4</span><span>현장 보고 {reports}건</span></div></article>;
}
...
function CrewHome({ brief, issueCount, onInvite, onWork }: {
  ...
  return (
    <div className="px-[var(--content-gutter)] pb-28 pt-4">
      ...
      <ActiveMoveCard heading="진행 중인 이사 1건" leading={<span className="grid size-12 shrink-0 place-items-center rounded-full bg-primary-50 text-primary-700"><Truck aria-hidden="true" size="var(--icon-md)" /></span>} meta={startAt ? `${dayFormatter.format(startAt)} · ${timeFormatter.format(startAt)}` : "일정 확인 중"} onOpen={onWork} route={<>{brief?.masked_origin ?? "출발지"} → {brief?.masked_destination ?? "도착지"}</>}><MoveJourneyProgress current={2} /><div className="mt-3 border-t border-line pt-3"><button className="w-full text-left" onClick={onWork} type="button"><strong className="block text-ui-support text-primary-700">작업범위 확인</strong><span className="mt-0.5 block text-ui-data text-ink-600">최신 승인본과 현장 조건을 확인해요</span><span className="mt-2 flex min-h-10 w-full items-center justify-center rounded-xl bg-primary-600 text-ui-support font-extrabold text-white">지금 확인</span></button></div></ActiveMoveCard>
      <aside className="mt-4 flex gap-3 rounded-[var(--radius-card)] border border-warning bg-warning-bg p-4 text-sm font-bold leading-6 text-warning-ink"><WarningCircle aria-hidden="true" className="mt-0.5 shrink-0" /> 현장이 승인본과 다르면 추가 금액을 요구하지 말고 먼저 보고해 주세요. {issueCount > 0 ? `현재 보고 ${issueCount}건이 처리 중이에요.` : ""}</aside>
    </div>
  );
}
...
function CrewCompletion({ brief, connection, issues }: {
  ...
  return <div className="space-y-4 px-[var(--content-gutter)] pb-28 pt-4">
    ...
    <section className="rounded-[var(--radius-card)] border border-line bg-surface px-4 py-1">{items.map((item) => { const selected = isCompleted(item.key, item.confirmed); return <button aria-pressed={selected} className="press-static flex min-h-14 w-full items-center gap-3 text-left" key={item.key} onClick={() => setCompletionState((current) => ({ ...current, [item.key]: !selected }))} type="button"><span className={`grid size-7 shrink-0 place-items-center rounded-full border-2 ${selected ? "border-primary-600 bg-primary-600 text-white" : "border-line text-transparent"}`}><Check aria-hidden="true" size="var(--icon-sm)" weight="bold" /></span><strong className="flex-1 text-base">{item.label}</strong></button>; })}</section>
    <section className="rounded-[var(--radius-card)] border border-line bg-surface p-4"><h2 className="text-ui-section font-black">완료 사진</h2><p className="mt-1 text-sm text-ink-600">작업 후 상태를 남겨주세요.</p><div className="mt-4 grid grid-cols-3 gap-2">{photos.map((url, index) => <img alt={`완료 사진 ${index + 1}`} className="aspect-square w-full rounded-xl object-cover" key={url} src={url} />)}{photos.length < 3 ? <label className="grid aspect-square cursor-pointer place-items-center rounded-xl border border-dashed border-primary-400 text-center text-sm font-extrabold text-primary-700"><span><Camera aria-hidden="true" className="mx-auto mb-1" size="var(--icon-md)" />사진 추가</span><input accept="image/*" capture="environment" className="sr-only" multiple onChange={addPhotos} type="file" /></label> : null}</div></section>
    <section className="rounded-[var(--radius-card)] border border-line bg-surface p-4"><h2 className="text-lg font-black">현장 보고</h2><p className="mt-2 text-sm text-success">보고 {issues.length}건이 확인서에 반영 또는 처리 중이에요.</p>{issues.slice(0, 2).map((issue) => <p className="mt-3 flex items-center justify-between border-t border-line pt-3 text-sm" key={issue.field_issue_id}><span>{issue.title}</span><span className="text-ink-600">{issueStatusLabel(issue.status)}</span></p>)}</section>
