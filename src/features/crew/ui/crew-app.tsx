...
function CrewCompletion({ brief, connection, issues }: {
  ...
  return <div className="space-y-4 px-[var(--content-gutter)] pb-28 pt-4">
    ...
    <section className="rounded-[var(--radius-card)] border border-line bg-surface px-4 py-1">{items.map((item) => { const selected = isCompleted(item.key, item.confirmed); return <button aria-pressed={selected} className="press-static flex min-h-14 w-full items-center gap-3 text-left" key={item.key} onClick={() => setCompletionState((current) => ({ ...current, [item.key]: !selected }))} type="button"><span className={`grid size-6 shrink-0 place-items-center rounded-full border-2 ${selected ? "border-primary-600 bg-primary-600 text-white" : "border-line text-transparent"}`}><Check aria-hidden="true" size="var(--icon-xs)" weight="bold" /></span><strong className="flex-1 text-base">{item.label}</strong></button>; })}</section>
