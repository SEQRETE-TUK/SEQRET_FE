...
export function CrewIssueReport({ brief, connection, issues }: {
  ...
  const evidenceReadQueries = useQueries({
    queries: sentIssues.map((issue) => {
      ...
      return {
        ...
        refetchInterval: (query: { state: { data?: unknown; fetchFailureCount: number } }) => query.state.data || query.state.fetchFailureCount >= 5 ? false : 2_000,
      };
    }),
  });
  ...
  if (!checkedIn) {
    return <div className="space-y-2 bg-canvas pb-28 pt-2"><section className="bg-surface px-[var(--content-gutter)] py-5"><p className="text-sm font-extrabold text-primary-700">현장 도착 확인</p><h2 className="mt-2 text-ui-component font-black">보고 전 체크인이 필요해요</h2><div className="mt-4 space-y-2">{brief?.check_in_items.map((item) => <label className="flex min-h-14 items-center gap-3 rounded-[var(--radius-component)] border border-line px-4" key={item.key}><input checked={item.confirmed || checkKeys.includes(item.key)} onChange={(event) => setCheckKeys((current) => event.target.checked ? [...current, item.key] : current.filter((key) => key !== item.key))} type="checkbox" /><span className="text-sm font-bold">{item.label}</span></label>)}</div><Button className="mt-4 w-full" disabled={!allChecked || checkInMutation.isPending} onClick={() => checkInMutation.mutate()} size="cta">현장 도착 체크인</Button></section>{error ? <div className="px-[var(--content-gutter)]"><ErrorToast message={apiErrorMessage(error)} /></div> : null}</div>;
  }

  if (formOpen) {
    return <Sheet onOpenChange={setFormOpen} open={formOpen}><SheetContent presentation="page" showClose={false}><MobilePageHeader onBack={() => setFormOpen(false)} title="새 현장 보고" /><SheetTitle className="sr-only">새 현장 보고</SheetTitle><SheetDescription className="sr-only">현장 사실과 증거를 업체에 보고합니다.</SheetDescription><div className="space-y-2 bg-canvas pb-28 pt-2">
        <section className="bg-surface px-[var(--content-gutter)] py-5"><ChoiceGroup columns={3} label="어떤 문제인가요?" onChange={(label) => setIssueType(issueTypeOptions.find((option) => option.label === label)?.value ?? "site_blocker")} options={issueTypeOptions.map((option) => option.label)} value={issueTypeOptions.find((option) => option.value === issueType)?.label ?? "현장 장애"} /></section>
        <section className="bg-surface px-[var(--content-gutter)] py-5"><Label className="block text-ui-component" htmlFor="issue-title">보고 내용</Label><Input className="mt-4" id="issue-title" maxLength={100} onChange={(event) => setTitle(event.target.value)} placeholder="예: 엘리베이터 운행 중단" value={title} /><Label className="mt-5 block text-sm font-extrabold" htmlFor="issue-description">현장 설명</Label><Textarea className="mt-2 min-h-28" id="issue-description" maxLength={200} onChange={(event) => setDescription(event.target.value)} placeholder="무엇이 달라졌고 작업에 어떤 영향이 있는지 적어주세요." value={description} /><p className="mt-1 text-right text-xs text-ink-400">{description.length}/200</p></section>
        <section className="bg-surface px-[var(--content-gutter)] py-5"><h3 className="text-ui-component">현장 증거</h3><div className="mt-4 grid grid-cols-3 gap-2">{evidence.map((item, index) => <div className="relative" key={item.id}><img alt={`현장 증거 ${index + 1}`} className="aspect-square w-full rounded-[var(--radius-component)] object-cover" src={item.url} /><button aria-label={`현장 증거 ${index + 1} 삭제`} className="absolute right-1 top-1 grid size-9 place-items-center rounded-full bg-ink-900/75 text-lg text-white" onClick={() => removeEvidence(item)} type="button">×</button></div>)}{evidence.length < maxEvidenceCount ? <Label className="grid aspect-square cursor-pointer place-items-center rounded-[var(--radius-component)] border border-dashed border-primary-300 bg-primary-50 text-center text-sm font-extrabold text-primary-700"><span><Plus aria-hidden="true" className="mx-auto mb-1" size="var(--icon-md)" />{uploadMutation.isPending ? "업로드 중…" : "사진 추가"}</span><input accept="image/jpeg,image/png" capture="environment" className="sr-only" disabled={uploadMutation.isPending} multiple onChange={pickEvidence} type="file" /></Label> : null}</div></section>
        <div className="px-[var(--content-gutter)]"><InfoCallout icon={<WarningCircle aria-hidden="true" />}>기사는 현장 사실만 보고하며 금액을 입력하지 않아요.</InfoCallout>{error ? <ErrorToast message={apiErrorMessage(error)} /> : null}</div>
  ...
  return <div className="space-y-2 bg-canvas pb-28 pt-2">
    <section className="bg-surface px-[var(--content-gutter)] py-5 text-center"><span className="mx-auto grid size-12 place-items-center rounded-full bg-primary-50 text-primary-700"><ClipboardText aria-hidden="true" size="var(--icon-md)" weight="duotone" /></span><h2 className="mt-4 text-ui-component font-black">승인본과 다른 점이 있나요?</h2><p className="mt-2 text-sm leading-6 text-ink-600">현장 사실과 사진을 업체에 보내면 업체가 변경 범위와 금액을 제안해요.</p><Button className="mt-4 w-full" onClick={() => setFormOpen(true)} size="cta"><Camera aria-hidden="true" /> 새 현장 보고</Button></section>
      <section className="bg-surface px-[var(--content-gutter)] py-5"><h2 className="text-ui-component font-black">내가 보낸 보고 <span className="text-primary-700">{sentIssues.length}건</span></h2><div className="mt-4 divide-y divide-line">{sentIssues.map((issue, index) => { const evidenceQuery = evidenceReadQueries[index]; return <button aria-label={`${issue.title} 상세 보기`} className="flex min-h-20 w-full items-center gap-3 text-left hover:bg-surface-muted" key={issue.field_issue_id} onClick={() => setSelectedIssueId(issue.field_issue_id)} type="button">{mockApiEnabled ? <img alt={`${issue.title} 현장 증거`} className="size-11 shrink-0 rounded-xl object-cover" src="/elevator-outage-evidence.png" /> : evidenceQuery?.data?.read_url ? <img alt={`${issue.title} 현장 증거`} className="size-11 shrink-0 rounded-xl object-cover" src={evidenceQuery.data.read_url} /> : <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-warning-bg text-warning-ink"><WarningCircle aria-hidden="true" size="var(--icon-md)" /></span>}<span className="min-w-0 flex-1"><strong className="block truncate text-ui-support">{issue.title}</strong><span className="mt-1 block text-xs text-ink-600">증거 {issue.evidence_media_asset_ids.length}건 · {new Intl.DateTimeFormat("ko-KR", { hour: "numeric", minute: "2-digit" }).format(new Date(issue.reported_at))}</span></span><span className="shrink-0 text-xs font-extrabold text-primary-700">{issueStatusLabel(issue.status)}</span><CaretRight aria-hidden="true" className="shrink-0 text-ink-400" size="var(--icon-sm)" /></button>; })}</div></section>
      {selectedIssue ? <Sheet onOpenChange={(open) => { if (!open) setSelectedIssueId(null); }} open><SheetContent className="!bg-ink-900 [&>button]:bg-ink-900/70 [&>button]:text-white [&>button]:hover:bg-ink-900/90">{mockApiEnabled ? <img alt={`${selectedIssue.title} 현장 증거`} className="aspect-[16/10] w-full object-cover" src="/elevator-outage-evidence.png" /> : selectedEvidenceReadQuery?.data?.read_url ? <img alt={`${selectedIssue.title} 현장 증거`} className="aspect-[16/10] w-full object-cover" src={selectedEvidenceReadQuery.data.read_url} /> : selectedEvidenceReadQuery?.isPending ? <div className="grid aspect-[16/10] place-items-center px-6 text-center text-sm text-white/80">증거 사진을 불러오는 중이에요.</div> : <div className="grid aspect-[16/10] place-items-center px-6 text-center text-sm text-white/80">증거 사진을 불러오지 못했어요.</div>}<div className="bg-surface px-4 pt-5"><SheetHeader className="px-0 pb-4 pr-0"><SheetTitle>{selectedIssue.title}</SheetTitle><SheetDescription>{issueStatusLabel(selectedIssue.status)} · 증거 {selectedIssue.evidence_media_asset_ids.length}건</SheetDescription></SheetHeader><p className="pb-6 text-sm leading-6 text-ink-600">{selectedDescription}</p></div></SheetContent></Sheet> : null}
    {submitted ? <p className="mx-[var(--content-gutter)] flex items-center gap-2 rounded-xl bg-success-bg p-4 text-sm font-extrabold text-success-ink"><CheckCircle aria-hidden="true" weight="fill" /> 업체에 현장 이슈를 전송했습니다.</p> : null}
    <div className="px-[var(--content-gutter)]"><InfoCallout icon={<WarningCircle aria-hidden="true" />}>기사는 현장 사실만 보고하며 금액을 입력하지 않아요.</InfoCallout></div>
  </div>;
}
