...
export function CrewIssueReport({ brief, connection, issues }: {
  ...
  if (!checkedIn) {
    return <div className="px-[var(--content-gutter)] pb-28 pt-5"><section className="rounded-[var(--radius-card)] border border-line bg-surface p-5"><p className="text-sm font-extrabold text-primary-700">현장 도착 확인</p><h2 className="mt-2 text-ui-section font-black">보고 전 체크인이 필요해요</h2><div className="mt-5 space-y-2">{brief?.check_in_items.map((item) => <label className="flex min-h-14 items-center gap-3 rounded-xl border border-line px-4" key={item.key}><input checked={item.confirmed || checkKeys.includes(item.key)} onChange={(event) => setCheckKeys((current) => event.target.checked ? [...current, item.key] : current.filter((key) => key !== item.key))} type="checkbox" /><span className="text-sm font-bold">{item.label}</span></label>)}</div><Button className="mt-5 w-full" disabled={!allChecked || checkInMutation.isPending} onClick={() => checkInMutation.mutate()} size="cta">현장 도착 체크인</Button></section>{error ? <p className="mt-3 text-sm font-bold text-danger-ink" role="alert">{apiErrorMessage(error)}</p> : null}</div>;
  }
  ...
  if (formOpen) {
    return <div className="px-[var(--content-gutter)] pb-28 pt-4">
      ...
      <section className="mt-2 rounded-[var(--radius-card)] border border-line bg-surface p-4">
