        ...
        <main className="flex flex-1 flex-col px-[var(--content-gutter)] pb-40 pt-10" id="main-content">
          <div className="my-auto w-full">
            <section aria-labelledby="role-entry-title">
              <h1 className="text-ui-section font-black tracking-[var(--tracking-display)]" id="role-entry-title">{entryStep === "name" ? "어떻게 불러드릴까요?" : entryStep === "code" ? "이사 연결 코드로 들어갈게요" : entryStep === "choice" ? "어떻게 시작할까요?" : "어떤 역할로 시작할까요?"}</h1>
            </section>

            <div className="pt-4">
              {entryStep === "name" ? <div className="pt-4"><label className="sr-only" htmlFor="customer-display-name">이름</label><Input autoComplete="name" autoFocus id="customer-display-name" maxLength={100} onChange={(event) => setDisplayName(event.target.value)} placeholder="이름 입력" value={displayName} /></div> : entryStep === "code" ? <div className="space-y-3 pt-4"><p className="text-ui-support text-ink-600">이사 연결 코드로 기존 이사에 다시 들어갑니다.</p><label className="sr-only" htmlFor="role-access-code">이사 연결 코드</label><Input autoCapitalize="characters" autoComplete="one-time-code" autoFocus id="role-access-code" onChange={(event) => setAccessCode(event.target.value)} placeholder="MOVE-XXXXXXXX" spellCheck={false} value={accessCode} />{error ? <ErrorToast message={error} /> : null}</div> : entryStep === "choice" ? <div className="grid grid-cols-2 gap-3 pt-4"><Button className="h-auto min-h-0 flex-col items-stretch justify-start gap-0 rounded-[var(--radius-feature)] border-line bg-surface p-3 text-left shadow-[var(--shadow-card)] hover:border-primary-300 hover:bg-surface" onClick={() => setEntryStep("name")} size="cta" type="button" variant="outline"><span className="grid h-28 place-items-center overflow-hidden rounded-[var(--radius-card)] p-2"><img alt="" aria-hidden="true" className="size-20 object-contain" decoding="async" src="/moving-items/moving-box.png" /></span><span className="mt-3 block text-ui-component font-black text-ink-900">새 이사</span><span className="mt-1 block text-ui-data font-normal leading-5 text-ink-600">이사 정보를 입력해<br />새로 시작해요</span></Button><Button className="h-auto min-h-0 flex-col items-stretch justify-start gap-0 rounded-[var(--radius-feature)] border-line bg-surface p-3 text-left shadow-[var(--shadow-card)] hover:border-primary-300 hover:bg-surface" onClick={() => { setAccessCode(mockApiEnabled ? mockConnectionCodes.main : ""); setError(null); setEntryStep("code"); }} size="cta" type="button" variant="outline"><span className="grid h-28 place-items-center overflow-hidden rounded-[var(--radius-card)] p-2"><img alt="" aria-hidden="true" className="size-20 object-contain" decoding="async" src="/moving-items/sofa.png" /></span><span className="mt-3 block text-ui-component font-black text-ink-900">이사 연결 코드</span><span className="mt-1 block text-ui-data font-normal leading-5 text-ink-600">기존 이사 상황을<br />불러와요</span></Button></div> : <fieldset>
