...
export function RoleEntry() {
  ...
  const [entryStep, setEntryStep] = useState<"role" | "name">("role");
  ...
  const start = () => {
    if (entryStep === "role") {
      if (selected === "customer") setEntryStep("name");
      else {
        clearSession();
    ...
    <div className="mobile-stage">
      <MobileFrame className="flex min-h-dvh flex-col bg-canvas">
        ...
        <main className="flex flex-1 flex-col px-[var(--content-gutter)] pb-40 pt-10" id="main-content">
          <div className="my-auto w-full">
            <section aria-labelledby="role-entry-title">
              <h1 className="text-ui-section font-black tracking-[var(--tracking-display)]" id="role-entry-title">{entryStep === "name" ? "어떻게 불러드릴까요?" : "어떤 역할로 시작할까요?"}</h1>
            </section>
        ...
        <div className="app-fixed-action fixed inset-x-0 bottom-0 z-[var(--z-sticky)] mx-auto w-full max-w-[var(--shell-mobile)] px-[var(--content-gutter)] pt-3">
          {entryStep === "role" ? <Button className="w-full" onClick={start} size="cta">{selectedRole.startLabel}<ArrowRight aria-hidden="true" /></Button> : <div className="grid grid-cols-[92px_minmax(0,1fr)] gap-2"><Button onClick={() => setEntryStep("role")} size="cta" variant="secondary">이전</Button><Button className="w-full" disabled={!displayName.trim()} onClick={start} size="cta">시작<ArrowRight aria-hidden="true" /></Button></div>}
        </div>
      </MobileFrame>
    </div>
  );
