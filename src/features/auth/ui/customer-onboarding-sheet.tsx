...
export function CustomerOnboardingSheet({ onOpenChange, open }: { onOpenChange: (open: boolean) => void; open: boolean }) {
  ...
  return <Sheet onOpenChange={close} open={open}><SheetContent className="flex !transition-none !transform-none flex-col" presentation="page" showClose={false}><MobilePageHeader onBack={() => close(false)} title={step === "schedule" ? "서비스 희망일" : step === "origin" ? "출발지 정보" : "도착지 정보"} />{error ? <ErrorToast message={error} /> : null}<form className="flex flex-1 flex-col" onSubmit={submit}>{step === "schedule" ? <CalendarStep month={month} onMonth={setMonth} onSelect={setSelectedDate} onTime={setTime} selectedDate={selectedDate} time={time} /> : step === "origin" ? <StopStep kind="origin" onChange={setOrigin} value={origin} /> : <StopStep kind="destination" onChange={setDestination} value={destination} />}<SheetFooter className="mt-auto grid gap-2">{step === "destination" ? <Button className="w-full" disabled={!valid || creating} key="submit" type="submit">{creating ? <><LoaderCircle aria-hidden="true" className="animate-spin" />초안 만드는 중</> : "이사 초안 만들기"}</Button> : <Button className="w-full" disabled={!valid} key="next" onClick={next} type="button">다음</Button>}</SheetFooter></form></SheetContent></Sheet>;
}
