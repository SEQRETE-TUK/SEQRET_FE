...
export function AddressSearchInput({ id, onChange, value }: { id: string; onChange: (value: string) => void; value: string }) {
  ...
  return <div>
    <div className="grid grid-cols-[minmax(0,1fr)_96px] gap-2">
      ...
      <button className="inline-flex min-h-11 items-center justify-center gap-1 rounded-[var(--radius-control)] border border-primary-400 bg-surface px-3 text-sm font-extrabold text-primary-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring" disabled={loading} onClick={search} type="button"><MagnifyingGlass aria-hidden="true" size="var(--icon-sm)" />{loading ? "불러오는 중" : "주소 검색"}</button>
    </div>
