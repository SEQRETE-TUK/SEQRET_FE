    ...
    <div className="pb-[var(--bottom-rail-height)]">
      ...
      <div className="mx-[var(--content-gutter)]"><ActiveMoveCard heading="진행 중인 이사 1건" leading={<span className="grid size-12 shrink-0 place-items-center rounded-full bg-primary-50 text-primary-700"><Truck aria-hidden="true" size="var(--icon-md)" /></span>} meta={<>{scheduledAt ? dateFormatter.format(scheduledAt) : "일정 확인 중"} {dDay !== null ? <b className="text-primary-700">· D-{dDay}</b> : null}</>} onOpen={onOpenMove} route={scope ? `${scope.job.origin_summary ?? "출발지"} → ${scope.job.destination_summary ?? "도착지"}` : "이사 정보를 불러오는 중"}>
        ...
        <div className="mt-3 border-t border-line pt-3">
          <button className="w-full text-left" onClick={onOpenAgreement} type="button"><strong className="block text-ui-support text-primary-700">{actionTitle}</strong><span className="mt-0.5 block text-ui-data text-ink-600">{actionMeta}</span><span className="mt-2 flex min-h-10 w-full items-center justify-center rounded-xl bg-primary-600 text-ui-support font-extrabold text-white">지금 확인</span></button>
        </div>
      </ActiveMoveCard></div>
...
function NewMoveHero({ onStart }: { onStart: () => void }) {
  return <button className="press-static relative min-h-[174px] w-full overflow-hidden rounded-[var(--radius-feature)] bg-primary-50 px-5 py-4 text-left ring-1 ring-primary-100" onClick={onStart} type="button"><span className="relative z-10 block max-w-[62%]"><strong className="block text-ui-section leading-7 font-extrabold tracking-[var(--tracking-display)]">60초 촬영으로<br />준비를 시작해요</strong><span className="mt-1.5 block text-ui-data leading-5 text-ink-600">짐 목록과 작업조건 초안을 만들어요</span><span className="mt-3 flex items-center gap-1 whitespace-nowrap text-sm font-extrabold text-primary-700">새 이사 시작하기 <ArrowRight aria-hidden="true" size="var(--icon-sm)" weight="bold" /></span></span><img alt="휴대폰으로 이삿짐 상자를 촬영하는 모습" className="absolute -right-6 -bottom-3 w-[170px] max-w-none" height="213" src="/move-capture-hero.png" width="170" /></button>;
}

function PreventionSection() {
  return <section className="mt-4 bg-primary-50/70 px-[var(--content-gutter)] py-5"><h2 className="text-ui-section font-black tracking-[var(--tracking-display)]">추가금이 생기는 순간</h2><p className="mt-1 text-sm text-ink-600">미리 확인하면 당일 변경을 줄일 수 있어요</p><div className="no-scrollbar mt-4 flex snap-x gap-3 overflow-x-auto pb-1"><PreventionCard iconSrc="/prevention-condition.svg" label="작업조건 차이" title="엘리베이터·계단 조건이 달랐어요" /><PreventionCard iconSrc="/prevention-inventory.svg" label="짐 목록 차이" title="촬영 후 큰 짐이 추가됐어요" /><PreventionCard iconSrc="/prevention-work.svg" label="추가 작업" title="분해·설치 작업이 빠졌어요" /></div></section>;
}

function GuestHome({ onStart }: { onStart: () => void }) {
  return <div className="pb-[var(--bottom-rail-height)]"><section className="px-[var(--content-gutter)]"><NewMoveHero onStart={onStart} /></section><section className="mx-[var(--content-gutter)] mt-4 rounded-[var(--radius-feature)] border border-line bg-surface p-5 shadow-[var(--shadow-card)]"><h2 className="text-ui-component font-black">진행 중인 이사가 없어요</h2><p className="mt-2 text-sm leading-6 text-ink-600">새 이사를 만들면 촬영부터 공동확인까지 한곳에서 이어집니다.</p></section><PreventionSection /></div>;
}

function PreventionCard({ iconSrc, label, title }: { iconSrc: string; label: string; title: string }) {
  return <article className="flex min-h-[176px] w-[170px] shrink-0 snap-start flex-col rounded-[var(--radius-card)] border border-line bg-surface p-4 shadow-[var(--shadow-card)]"><span className="grid size-14 place-items-center rounded-2xl bg-surface-muted"><img alt="" aria-hidden="true" className="size-11 object-contain" height="44" loading="lazy" src={iconSrc} width="44" /></span><div className="mt-auto pt-4"><p className="text-xs font-extrabold text-primary-700">{label}</p><h3 className="mt-1.5 text-ui-support leading-5 font-extrabold">{title}</h3></div></article>;
}

function EmptyMoveList({ onNewMove }: { onNewMove: () => void }) {
  return <div className="px-[var(--content-gutter)] pb-28 pt-5"><div className="flex items-center justify-between gap-3"><h1 className="text-ui-section">내 이사</h1><button className="min-h-11 whitespace-nowrap px-2 text-sm font-semibold text-primary-700" onClick={onNewMove} type="button">+ 새 이사</button></div><div className="mt-6 grid grid-cols-2 border-b border-line" role="tablist" aria-label="이사 목록"><button aria-selected="true" className="relative min-h-11 text-sm font-semibold text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" role="tab" type="button">진행 중 0</button><button aria-selected="false" className="min-h-11 text-sm font-semibold text-ink-600" role="tab" type="button">기록 0</button></div><section className="mt-8 rounded-[var(--radius-feature)] border border-line bg-surface px-5 py-8 text-center shadow-[var(--shadow-card)]"><Archive aria-hidden="true" className="mx-auto text-ink-400" size="var(--icon-category)" /><h2 className="mt-4 text-lg font-black">아직 이사 기록이 없어요</h2><p className="mt-2 text-sm leading-6 text-ink-600">새 이사를 만들면 진행 상황과 최종 기록을 확인할 수 있어요.</p><Button className="mt-5 w-full" onClick={onNewMove}>새 이사 시작</Button></section></div>;
}
...
function ConsumerMoveList({ completion, onNewMove, onOpen, scope }: { completion: CompletionSummary | undefined; onNewMove: () => void; onOpen: () => void; scope: ScopeReview | undefined }) {
  ...
  return <div className="px-[var(--content-gutter)] pb-28 pt-5">
    <div className="flex items-center justify-between gap-3"><h1 className="text-ui-section font-black tracking-[var(--tracking-display)]">내 이사</h1><button className="min-h-11 whitespace-nowrap px-2 text-sm font-extrabold text-primary-700" onClick={onNewMove} type="button">+ 새 이사</button></div>
    <div className="mt-6 grid grid-cols-2 border-b border-line" role="tablist" aria-label="이사 목록">
      <button aria-selected={listTab === "active"} className={`relative min-h-11 text-sm font-semibold ${listTab === "active" ? "text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" : "text-ink-600"}`} onClick={() => setListTab("active")} role="tab" type="button">진행 중 {completed ? 0 : 1}</button>
      <button aria-selected={listTab === "history"} className={`relative min-h-11 text-sm font-semibold ${listTab === "history" ? "text-primary-700 after:absolute after:inset-x-6 after:bottom-0 after:h-0.5 after:bg-primary-600" : "text-ink-600"}`} onClick={() => setListTab("history")} role="tab" type="button">기록 {historyCount}</button>
    </div>
    {showCurrentMove ? <button className="press-static mt-5 w-full rounded-[var(--radius-feature)] border border-line bg-surface p-5 text-left shadow-[var(--shadow-card)]" onClick={onOpen} type="button">
      ...
      <strong className="mt-4 flex items-center justify-between gap-3 text-ui-section tracking-[var(--tracking-display)]"><span className="min-w-0 truncate">{scope ? `${scope.job.origin_summary ?? "출발지"} → ${scope.job.destination_summary ?? "도착지"}` : "이사 정보를 불러오는 중"}</span><CaretRight aria-hidden="true" className="shrink-0 text-ink-400" size="var(--icon-sm)" /></strong>
    ...
    </button> : listTab === "history" && mockApiEnabled ? <div className="mt-5 space-y-3"><MoveHistoryCard amount="1,430,000원" changes={1} date="2026년 5월 18일" destination="자양동 오피스텔" items={21} origin="성수동 원룸" version="v4" /><MoveHistoryCard amount="620,000원" changes={0} date="2025년 8월 25일" destination="성수동 원룸" items={12} origin="건대입구 원룸" version="v2" /></div> : <section className="mt-5 rounded-[var(--radius-feature)] border border-line bg-surface px-5 py-8 text-center"><Archive aria-hidden="true" className="mx-auto text-ink-400" size="var(--icon-category)" /><h2 className="mt-3 font-black">{listTab === "active" ? "진행 중인 이사가 없어요" : "완료된 이사가 없어요"}</h2></section>}
  </div>;
}
...
function MoveHistoryCard({ amount, changes, date, destination, items, origin, version }: { amount: string; changes: number; date: string; destination: string; items: number; origin: string; version: string }) {
  return <article className="rounded-[var(--radius-feature)] border border-line bg-surface p-5"><span className="inline-flex rounded-full bg-success-bg px-3 py-1 text-xs font-extrabold text-success-ink"><Check aria-hidden="true" className="mr-1" size="var(--icon-xs)" weight="bold" />완료</span><h2 className="mt-4 text-ui-section font-black tracking-[var(--tracking-display)]">{origin} → {destination}</h2><p className="mt-2 text-sm text-ink-600">{date}</p><p className="mt-4 border-t border-line pt-4 text-sm">최종 확인서 <strong>{version}</strong><span className="mx-2 text-line">|</span>최종 금액 <strong className="text-primary-700">{amount}</strong></p><div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-ink-600"><span className="inline-flex items-center gap-1"><Package aria-hidden="true" size="var(--icon-sm)" />짐 {items}개</span><span className="inline-flex items-center gap-1"><Notepad aria-hidden="true" size="var(--icon-sm)" />확인서 {version}</span><span className="inline-flex items-center gap-1"><TrendUp aria-hidden="true" size="var(--icon-sm)" />변경 {changes}건</span></div></article>;
}
...
function MoveInfo({ onChange, scope, value }: { onChange: (value: MoveInfoOverrides) => void; scope: ScopeReview | undefined; value: MoveInfoOverrides }) {
  ...
  return (
    <div className="space-y-2.5 px-[var(--content-gutter)] pb-28 pt-3">
      <InfoCard title="이사 일정"><div className="flex items-center gap-3"><Calendar aria-hidden="true" className="shrink-0" size="var(--icon-category)" /><strong className="min-w-0 flex-1 text-base leading-5">{scheduledAt ? fullDateFormatter.format(scheduledAt) : "일정을 입력해 주세요"}</strong>{dDay !== null ? <span className="whitespace-nowrap rounded-lg bg-primary-50 px-2 py-1.5 text-ui-data font-extrabold text-primary-700">D-{dDay}</span> : null}<button className="min-h-11 whitespace-nowrap px-1 text-sm font-extrabold text-primary-700" onClick={openScheduleEditor} type="button">수정</button></div></InfoCard>
...
function InfoCard({ children, title }: { children: ReactNode; title: string }) { return <section className="rounded-[var(--radius-card)] border border-line bg-surface p-3 shadow-[var(--shadow-card)]"><h2 className="mb-2.5 text-base font-black">{title}</h2>{children}</section>; }
...
function RoutePoint({ destination = false, label, onEdit, stop }: { destination?: boolean; label: string; onEdit: () => void; stop: MoveStopDraft }) {
  ...
  return <div className={destination ? "relative mt-6" : "relative"}><span aria-hidden="true" className="absolute top-1 -left-[46px] size-5 rounded-full border-[5px] border-primary-600 bg-surface" /><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="text-ui-data font-extrabold text-primary-700">{label}</p><strong className="mt-0.5 block text-ui-support">{stop.address || `${label}를 입력해 주세요`}</strong>{stop.detailAddress ? <span className="mt-0.5 block text-xs text-ink-600">{stop.detailAddress}</span> : null}</div><button className="min-h-11 whitespace-nowrap px-1 text-sm font-extrabold text-primary-700" onClick={onEdit} type="button">수정</button></div><div className="mt-1.5 flex flex-wrap gap-1.5">{conditions.map((condition, index) => <span className={`rounded-full px-2.5 py-0.5 text-ui-micro ${index === conditions.length - 1 && stop.parking === "확인 필요" ? "bg-warning-bg text-warning-ink" : "bg-surface-muted text-ink-600"}`} key={condition}>{condition}</span>)}</div></div>;
}
...
function ConsumerInventory({ onCapture, onManualAdd, scope }: { onCapture: () => void; onManualAdd: () => void; scope: ScopeReview | undefined }) {
  ...
  return <div className="px-[var(--content-gutter)] pb-24 pt-4">
    <label className="flex min-h-12 items-center gap-2 rounded-xl border border-line bg-surface px-3 focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-focus-ring" htmlFor="inventory-search"><MagnifyingGlass aria-hidden="true" className="shrink-0 text-ink-400" size="var(--icon-md)" /><input className="min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-ink-400" id="inventory-search" onChange={(event) => setQuery(event.target.value)} placeholder="짐 이름 검색" type="search" value={query} /></label>
...
function FieldReportCard({ companyName, issue, onOpen }: { companyName: string | null; issue: FieldIssue; onOpen: () => void }) {
  ...
  return <section className="rounded-[var(--radius-card)] border border-line bg-surface p-3"><span className={`inline-flex rounded-full border px-2.5 py-0.5 text-ui-micro font-extrabold ${resolved ? "border-success text-success" : "border-warning text-warning-ink"}`}>{statusLabel}</span><h2 className="mt-1.5 text-lg font-black">현장 보고 <b className="text-primary-700">1건</b>이 도착했어요</h2><p className="mt-0.5 text-ui-data text-ink-600">{issue.title}</p><p className="mt-1 line-clamp-2 text-xs text-ink-400">{issue.description}</p><div className="mt-2 flex items-center justify-between gap-3 text-ui-micro text-ink-400"><span className="min-w-0 truncate">현장기사 보고 · {companyName ?? "이사업체"} 기록</span><span className={`shrink-0 font-bold ${resolved ? "text-success" : "text-warning-ink"}`}>{responseLabel}</span></div><button className="mt-2 flex min-h-11 w-full items-center justify-between rounded-[var(--radius-control)] border border-line px-4 text-ui-data font-extrabold hover:bg-surface-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring" onClick={onOpen} type="button">보고 내용 확인 <CaretRight aria-hidden="true" size="var(--icon-sm)" /></button></section>;
}
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent className="!transition-none [&>button]:bg-ink-900/70 [&>button]:text-white [&>button]:hover:bg-ink-900/80" presentation="page">
    ...
    {loading ? <div className="grid min-h-dvh place-items-center text-sm text-ink-600">변경안을 불러오는 중입니다.</div> : <div>
      ...
      <div className="relative -mt-10 min-h-[calc(100dvh-300px)] rounded-t-[var(--radius-feature)] bg-surface px-5 pb-4 pt-8">
        <h3 className="text-center text-ui-section leading-8 font-black tracking-[var(--tracking-display)]">엘리베이터가 멈춰<br />계단 운반이 필요해요</h3>
        <p className="mt-3 text-center text-sm text-ink-600">{reportedAt} · 현장기사</p>
        {proposal ? <><section className="mt-7"><div className="flex items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-primary-50 text-primary-700"><Stairs aria-hidden="true" size="var(--icon-md)" /></span><h4 className="text-lg font-black">{proposal.title}</h4></div><dl className="mt-4 grid grid-cols-[88px_minmax(0,1fr)] gap-y-3 text-sm"><dt className="text-ink-600">사유</dt><dd>{proposal.reason}</dd><dt className="text-ink-600">예상 추가 시간</dt><dd>+1시간</dd></dl></section><section className="mt-5 grid grid-cols-3 divide-x divide-line rounded-xl border border-line py-4 text-center tabular-nums"><div><span className="text-xs text-ink-600">기존 금액</span><strong className="mt-1 block text-ui-support">{money(previous)}</strong></div><div><span className="text-xs text-ink-600">추가 금액</span><strong className="mt-1 block text-ui-support text-danger-ink">{adjustment > 0 ? "+" : ""}{money(adjustment)}</strong></div><div><span className="text-xs text-ink-600">변경 후</span><strong className="mt-1 block text-ui-support">{money(total)}</strong></div></section></> : <section className="mt-6 rounded-xl border border-line p-4"><h4 className="text-lg font-black">{issue.title}</h4><p className="mt-2 text-sm leading-6 text-ink-600">{issue.description}</p></section>}
        {error ? <p className="mt-3 text-sm font-bold text-danger">{apiErrorMessage(error)}</p> : null}
      </div>
    </div>}
    {!readOnly && proposal ? <SheetFooter className="grid gap-2"><Button className="w-full whitespace-nowrap" disabled={pending} onClick={() => onDecision("approve")} size="cta">변경 승인하기 · {adjustment > 0 ? "+" : ""}{money(adjustment)}</Button><Button className="w-full whitespace-nowrap" disabled={pending} onClick={() => setRequestOpen(true)} variant="outline"><ChatCircleDots aria-hidden="true" /> 수정 요청</Button></SheetFooter> : null}
