...
export function LiveProviderWorkflow({ embedded = false, wide = false }: { embedded?: boolean; wide?: boolean }) {
  ...
  return (
    <WorkflowShell
      ...
      wide={wide}
    >
      <div className="workflow-task-list overflow-hidden rounded-[var(--radius-component)] border border-line bg-surface">
        <WorkflowTask
        ...
        >
          <p className="mt-4 rounded-[var(--radius-component)] bg-primary-50 p-4 text-center font-mono text-lg font-black tracking-wide text-primary-800">{scopeQuery.data?.job.job_code ?? "코드 준비 중"}</p>
        ...
        >
          ...
          {dispatchQuery.data?.status === "confirmed" ? <div className="mt-4 rounded-[var(--radius-component)] bg-success-bg p-4 text-success-ink"><Check aria-hidden="true" className="inline" /> 배차 확정 · {dispatchQuery.data.confirmed_at ? eventTimeFormatter.format(new Date(dispatchQuery.data.confirmed_at)) : "시간 확인 중"}</div> : null}
...
function CompletionReview({ summary }: { summary: Awaited<ReturnType<typeof getCompletionSummary>> }) {
  ...
  return <div className="mt-4 space-y-4">
    <div className="rounded-[var(--radius-component)] bg-canvas p-4"><p className="text-ui-support text-ink-600">완료 기록 금액</p><strong className="text-ui-section tabular-nums">{money(summary.final_amount_krw)}</strong><p className="mt-2 text-ui-support text-ink-600">완료 제출 {summary.completion_submission_id ? "수신" : "대기"} · {requestStatus}</p></div>
    {summary.completion_submission_id ? <>
      ...
      {summary.completion_media.length ? <div><p className="text-sm font-extrabold">완료 사진</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">{summary.completion_media.map((asset, index) => asset.content_type.startsWith("image/") ? <img alt={`${asset.room_zone_label} 완료 사진 ${index + 1}`} className="aspect-square w-full rounded-[var(--radius-component)] object-cover" height="480" key={asset.media_asset_id} loading="lazy" src={asset.read_url} width="480" /> : <video className="aspect-square w-full rounded-[var(--radius-component)] bg-ink-900 object-cover" controls key={asset.media_asset_id} preload="metadata" src={asset.read_url} />)}</div></div> : <p className="rounded-[var(--radius-component)] bg-warning-bg p-3 text-sm font-bold text-warning-ink">완료 사진이 없습니다. 고객 요청 전에 기사에게 증빙을 확인해 주세요.</p>}
