...
import { Check, Download, LoaderCircle, Send, Truck, Users } from "lucide-react";
...
export function LiveProviderWorkflow({ wide = false }: { wide?: boolean }) {
    ...
    <WorkflowShell retryAfter={retryAfter} title="업체 운영" wide={wide}>
      ...
      {session.actor.invitation?.status === "pending" ? null : <div className={wide ? "grid gap-5 lg:grid-cols-2" : "space-y-5"}>
        <Card className="p-5">
          ...
          {scopeQuery.data ? <><p className="mt-3 text-sm text-ink-600">{scopeQuery.data.scope.version_label} · 항목 {scopeQuery.data.scope.item_count}개 · {scopeQuery.data.scope.status}</p><form className="mt-4 space-y-3" onSubmit={(event: FormEvent) => { event.preventDefault(); scopeMutation.mutate(); }}><Label htmlFor="quote-amount">총 견적 금액</Label><Input id="quote-amount" min="0" onChange={(event) => setQuoteAmount(event.target.value)} required type="number" value={quoteAmount} /><Label htmlFor="quote-reason">견적 사유</Label><Textarea id="quote-reason" maxLength={2000} onChange={(event) => setQuoteReason(event.target.value)} required value={quoteReason} /><Button className="w-full" disabled={!Number(quoteAmount) || !quoteReason.trim() || scopeMutation.isPending || scopeQuery.data.scope.status === "confirmed"} size="cta" type="submit"><Send /> 견적 제안 보내기</Button></form></> : <EmptyState>작업범위가 준비되면 견적을 보낼 수 있습니다.</EmptyState>}
          <ApiNotice error={scopeMutation.error} title="견적 제안을 처리하지 못했어요" />
        </Card>

        <Card className="p-5">
          <p className="text-sm font-bold text-warning-ink">현장 변경</p><h2 className="mt-1 text-xl font-extrabold">기사 이슈를 변경안으로</h2>
