...
export function ProviderIssueWorkbench({ connection, issues, scope }: { connection: Connection; issues: FieldIssue[]; scope: ScopeReview | undefined }) {
  ...
  return <div className="grid items-start gap-5 xl:grid-cols-[minmax(18rem,0.72fr)_minmax(0,1.28fr)]">
    <section aria-labelledby="provider-issue-list-title" className="ui-card ui-card-outlined overflow-hidden">
    ...
    <section className="ui-card ui-card-outlined ui-card-pad min-w-0">{selected && scope ? <IssueDetail key={selected.field_issue_id} connection={connection} issue={selected} scope={scope} /> : <p className="text-ui-support text-ink-600">처리할 현장 보고가 없습니다.</p>}</section>
  </div>;
}
  ...
  return <>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <div>
        ...
        {evidence.length ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{evidence.map((asset, index) => asset.content_type.startsWith("image/") ? <img alt={`${issue.title} 현장 증거 ${index + 1}`} className="aspect-[16/10] w-full rounded-[var(--radius-feature)] object-cover" height="400" key={asset.media_asset_id} loading="lazy" src={asset.read_url} width="640" /> : <video className="aspect-[16/10] w-full rounded-[var(--radius-feature)] bg-ink-900 object-cover" controls key={asset.media_asset_id} src={asset.read_url} />)}</div> : <div className="mt-3 grid aspect-[16/9] place-items-center rounded-[var(--radius-card)] bg-surface-muted px-5 text-center text-ui-support text-ink-600">증거 {issue.evidence_media_asset_ids.length}건이 연결됐지만 미리보기가 만료됐거나 아직 준비되지 않았습니다.</div>}
        <p className="mt-5 text-ui-control">기사 설명</p><p className="mt-2 break-words rounded-[var(--radius-card)] border border-line p-4 text-ui-support text-ink-600">{issue.description}</p>
        <h4 className="mt-5 text-ui-control">기준 승인 범위</h4><dl className="mt-2 space-y-2 rounded-[var(--radius-card)] border border-line p-4 text-ui-support"><div className="flex justify-between gap-3"><dt>포함 작업</dt><dd>{scope.scope.included_works.length}건</dd></div><div className="flex justify-between gap-3"><dt>제외 작업</dt><dd>{scope.scope.exclusions.length}건</dd></div><div className="flex justify-between gap-3"><dt>기준 금액</dt><dd>{money(scope.quote?.total_amount_krw)}</dd></div></dl>
      </div>
      <div>
        ...
        {issue.status === "open" ? <>
          ...
          <div className="rounded-[var(--radius-card)] border border-line p-4"><p className="text-ui-data text-ink-600">제안 작업</p><p className="mt-1 break-words text-ui-control">{proposal?.title ?? issue.title}</p><p className="mt-3 break-words text-ui-support text-ink-600">{proposal?.reason ?? "제안 내용을 불러오는 중…"}</p><p className="mt-4 flex justify-between gap-3 border-t border-line pt-4 text-ui-support"><span>변경 후 금액</span><strong className="text-primary-700">{money(resultingAmount)}</strong></p></div>
