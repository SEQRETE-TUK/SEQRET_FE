...
export function ManualScopeEditor({
  ...
  return (
    <form id={MANUAL_SCOPE_FORM_ID} onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
      <section>
        {(["가구", "가전", "기타"] as ManualCategory[]).map((category) => <section className="-mx-5 mt-2 bg-surface px-5 py-4 first:mt-0" key={category}><h3 className="text-[17px] leading-6 font-black">{category}</h3><div className="mt-3 grid grid-cols-4 gap-2">{movingItemCatalog.filter((item) => item.category === category).map(({ icon, label }) => { const active = selected.has(label); return <button aria-pressed={active} className={`press-static relative flex min-h-[96px] min-w-0 flex-col items-center justify-center rounded-xl px-0.5 text-center ${active ? "bg-primary-50 text-primary-800" : "bg-surface text-ink-900"}`} key={label} onClick={() => toggle(label)} type="button"><span className={`grid size-11 place-items-center ${manualCategoryTone[category]}`}><img alt="" aria-hidden="true" className="size-11 object-contain" src={icon} /></span><span className="mt-1 line-clamp-2 text-xs leading-4">{label}</span>{active ? <span className="absolute right-1.5 top-1.5 grid size-4 place-items-center rounded-full bg-primary-600 text-white"><Check aria-hidden="true" className="size-3" weight="bold" /></span> : null}</button>; })}</div></section>)}
      </section>
    </form>
  );
}
